odoo.define('odoo_studio_community.EditorMixin', function() {
"use strict";
...
return {
    ...
    handleDrop: function ($drag, node, position) {
        var isNew = $drag.hasClass('o_odoo_studio_community_component');
        var values;
        if (isNew) {
            values = {
                type: 'add',
    ...
    highlightNearestHook: function ($helper, position) {
        this.$('.o_odoo_studio_community_nearest_hook').removeClass('o_odoo_studio_community_nearest_hook');
    ...
    setSelectable: function ($el) {
        ...
        $el.click(function () {
            ...
            $(this).addClass('o_odoo_studio_community_clicked');
        })
        .mouseover(function () {
            if (self.$('.ui-draggable-dragging').length) {
                return;
            }
            $(this).addClass('o_odoo_studio_community_hovered');
        })
        .mouseout(function () {
            $(this).removeClass('o_odoo_studio_community_hovered');
        });
    },
    unselectedElements: function () {
        this.selected_node_id = false;
        var $el = this.$('.o_odoo_studio_community_clicked');
        $el.removeClass('o_odoo_studio_community_clicked');
        if ($el.find('.blockUI')) {
