odoo.define('odoo_studio_community.SearchEditor', function (require) {
"use strict";
...
var FormEditorHook = require('odoo_studio_community.FormEditorHook');
...
var SearchEditor = SearchRenderer.extend(EditorMixin, {
    ...
    className: SearchRenderer.prototype.className + ' o_odoo_studio_community_search_view_editor',
    ...
    highlightNearestHook: function ($helper, position) {
        ...
        var $nearest_form_hook = this.$('.o_odoo_studio_community_hook')
            .touching({
        ...
        if ($nearest_form_hook.length) {
            ...
            const isField = hookClasses.indexOf("o_odoo_studio_community_field") > -1;
            const isFilter = hookClasses.indexOf("o_odoo_studio_community_filter") > -1;
            const isSeparator = hookClasses.indexOf("o_odoo_studio_community_filter_separator") > -1;
            ...
            if (addHook) {
                $nearest_form_hook.addClass('o_odoo_studio_community_nearest_hook');
                return true;
            }
        }
        return false;
    ...
    _addHookEmptyTable: function () {
        var $tbody = this.$('.o_odoo_studio_community_search_autocompletion_fields tbody');
        if (!$tbody.children().length) {
            this._addFirstHook($tbody, 'field');
        }
        $tbody = this.$('.o_odoo_studio_community_search_filters tbody');
        if (!$tbody.children().length) {
            this._addFirstHook($tbody, 'filter');
        }
        $tbody = this.$('.o_odoo_studio_community_search_group_by tbody');
        if (!$tbody.children().length) {
    ...
    _render: function () {
        ...
        this.$('.ui-droppable').droppable({
            accept: ".o_odoo_studio_community_component",
            drop: function (event, ui) {
                var $hook = self.$('.o_odoo_studio_community_nearest_hook');
                if ($hook.length) {
                    ...
                    if (structure === "filter") {
                        ...
                        $hook.removeClass('o_odoo_studio_community_nearest_hook');
        ...
        this.$el.droppable({
            ...
            tolerance: "touch",
            over: function (ev, ui) {
                var $autocompletionFields = self.$('.o_odoo_studio_community_search_autocompletion_fields');
                var $filters = self.$('.o_odoo_studio_community_search_filters');
                var $grouBy = self.$('.o_odoo_studio_community_search_group_by');
