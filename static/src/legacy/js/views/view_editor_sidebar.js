...
import view_components from "odoo_studio_community.view_components";
import Widget from "web.Widget";
import { registry } from "@web/core/registry";
import { sortBy } from "@web/core/utils/arrays";
import { SIDEBAR_SAFE_FIELDS } from "@odoo_studio_community/legacy/js/views/sidebar_safe_fields";
...
export const ViewEditorSidebar = Widget.extend(StandaloneFieldManagerMixin, {
    template: 'odoo_studio_community.ViewEditorSidebar',
    events: {
        'click .o_odoo_studio_community_new:not(.inactive)':            '_onTab',
        'click .o_odoo_studio_community_view':                          '_onTab',
        'click .o_odoo_studio_community_xml_editor':                    '_onXMLEditor',
        'click .o_display_view .o_odoo_studio_community_parameters':    '_onViewParameters',
        'click .o_display_field .o_odoo_studio_community_parameters':   '_onFieldParameters',
        'click .o_display_view .o_odoo_studio_community_defaults':      '_onDefaultValues',
        'change #show_invisible':                            '_onShowInvisibleToggled',
        'click .o_odoo_studio_community_remove':                        '_onElementRemoved',
        'click .o_odoo_studio_community_restore':                       '_onRestoreDefaultView',
        'change .o_display_view input':                      '_onViewChanged',
        'change .o_display_view select':                     '_onViewChanged',
        'click .o_odoo_studio_community_edit_selection_values':         '_onSelectionValues',
        ...
        'click .o_odoo_studio_community_sidebar_approval .o_approval_archive':  '_onApprovalArchive',
        'change .o_odoo_studio_community_sidebar_approval':                     '_onApprovalChange',
        'click .o_odoo_studio_community_sidebar_approval .o_approval_domain':   '_onApprovalDomain',
        'click .o_odoo_studio_community_sidebar_approval .o_approval_new':      '_onApprovalNewRule',
        ...
        'click .o_odoo_studio_community_attrs':                         '_onDomainAttrs',
        'focus .o_display_filter input#domain':              '_onDomainEditor',
        'keyup .o_odoo_studio_community_sidebar_search_input':          '_onSearchInputChange',
        'click .o_odoo_studio_community_existing_fields_header':        '_onClickExistingFieldHeader',
    },
    /**
     * @constructor
     * @param {Widget} parent
    ...
    */
    ...
    _render: function () {
        ...
        if (this.state.mode === 'new') {
            ...
            return Promise.all(this.defs).then(() => {
                delete(this.defs);
                this.$('.o_odoo_studio_community_component').on("drag", _.throttle((event, ui) => {
    ...
    _renderComponentsSection: function () {
        ...
        $section.addClass('o_odoo_studio_community_new_components');
        const $sidebarContent = this.$('.o_odoo_studio_community_sidebar_content');
    ...
    _renderExistingFieldsSection: function () {
        const $existingFields = this.$('.o_odoo_studio_community_existing_fields');
        ...
        const $sidebarContent = this.$('.o_odoo_studio_community_sidebar_content');
        const $existingFieldsSection = $('<div/>', {class: `o_odoo_studio_community_existing_fields_section`});
        const $section = this._renderSection(formWidgets);
        $section.addClass('o_odoo_studio_community_existing_fields');
        if ($existingFields.length) {
            this.$('.o_odoo_studio_community_existing_fields_section').append($section);
        } else {
            const $sectionTitle = $('<h3>', {
                text: _t('Existing Fields'),
                class: 'o_odoo_studio_community_existing_fields_header',
            }).append($('<i/>', {class: `o_odoo_studio_community_existing_fields_icon fa fa-caret-right ms-2`}));
            const $sectionSubtitle = $('<h6>', {
                class: 'small text-white',
                text: _t('The following fields are currently not in the view.'),
            });
            const $sectionSearchDiv = core.qweb.render('odoo_studio_community.ExistingFieldsInputSearch');
    ...
    _renderNewFieldsSection: function () {
        ...
        const $section = this._renderSection(formWidgets);
        $section.addClass('o_odoo_studio_community_new_fields');
    ...
    _renderSection: function (form_widgets) {
        var self = this;
        var $components_container = $('<div>').addClass('o_odoo_studio_community_field_type_container');
    ...
    _updateExistingFieldSection() {
        const icon = this.el.querySelector('.o_odoo_studio_community_existing_fields_icon');
        const section = this.el.querySelector('.o_odoo_studio_community_existing_fields_section');
    ...
    _onSearchInputChange: function () {
        this._searchValue = this.$('.o_odoo_studio_community_sidebar_search_input').val();
    ...
    _onRestoreDefaultView: function () {
        ...
        Dialog.confirm(this, message, {
            ...
            dialogClass: 'o_odoo_studio_community_preserve_space'
        });
    },
    /**
     * @private
