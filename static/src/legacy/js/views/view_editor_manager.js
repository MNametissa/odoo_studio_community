odoo.define('odoo_studio_community.ViewEditorManager', function (require) {
"use strict";
...
var AbstractEditorManager = require('odoo_studio_community.AbstractEditorManager');
var bus = require('odoo_studio_community.bus');
var EditorMixin = require('odoo_studio_community.EditorMixin');
var EditorMixinOwl = require('odoo_studio_community.EditorMixinOwl');

var SearchEditor = require('odoo_studio_community.SearchEditor');
var SearchRenderer = require('odoo_studio_community.SearchRenderer');

var FieldSelectorDialog = require('odoo_studio_community.FieldSelectorDialog');
var NewButtonBoxDialog = require('odoo_studio_community.NewButtonBoxDialog');
var NewFieldDialog = require('odoo_studio_community.NewFieldDialog');
var utils = require('odoo_studio_community.utils');
const { ViewEditorSidebar } = require('@odoo_studio_community/legacy/js/views/view_editor_sidebar');
...
const { StudioView } = require("@odoo_studio_community/client_action/studio_view");
...
const { extendEnv } = require('@odoo_studio_community/client_action/view_editors/utils')
const { getNodesFromXpath, getLegacyNode, xpathToLegacyXpathInfo, serializeXmlToString, parseStringToXml, nodeStudioXpathSymbol } = require('@odoo_studio_community/client_action/view_editors/xml_utils')
...
async function wowlCreateInlineView(env, { subViewType, viewId, fullXpath, subViewRef, resModel, fieldName }) {
    ...
    const studioViewArch = await rpc('/odoo_studio_community/create_inline_view', {
        model: resModel,
        view_id: viewId,
...
var ViewEditorManager = AbstractEditorManager.extend(WidgetAdapterMixin, {
    ...
    updateEditor: function (options) {
        ...
        return this._instantiateEditor(options).then(function (editor) {
            ...
            return prom.then(function () {
                dom.append(self.$('.o_odoo_studio_community_view_renderer'), [fragment], {
                    in_DOM: self.isInDOM,
                ...
                def_field_values = new Promise(function (resolve, reject) {
                    ...
                    prom.then(function (openFieldDialog) {
                        if (!openFieldDialog) {
                            ...
                            var $message = $(QWeb.render('odoo_studio_community.FieldOne2manyWarning'));
    ...
    _applyChangeHandling: async function (result, opID) {
        ...
        // studio_view_id must be updated (but /odoo_studio_community/edit_view_arch
    ...
    _editField(modelName, fieldName, values, forceEdit) {
        return this._rpc({
            route: '/odoo_studio_community/edit_field',
            params: {
                model_name: modelName,
                field_name: fieldName,
                values: values,
    ...
    _editView: async function (view_id, studio_view_arch, operations) {
        ...
        return this._rpc({
            route: '/odoo_studio_community/edit_view',
            params: {
                view_id: view_id,
    ...
    _editViewArch: async function (view_id, view_arch) {
        core.bus.trigger('clear_cache');
        return this._rpc({
            route: '/odoo_studio_community/edit_view_arch',
            params: {
                view_id: view_id,
                view_arch: view_arch,
                // We write views in the base language to make sure we do it on the source term field
    ...
    _getDefaultValue: function (model_name, field_name) {
        return this._rpc({
            route: '/odoo_studio_community/get_default_value',
            params: {
    ...
    _getEmailAlias: function (model_name) {
        return this._rpc({
            route: '/odoo_studio_community/get_email_alias',
            params: {
                model_name: model_name,
            },
        });
    ...
    _renameField: function (oldName, newName) {
        ...
        return this._rpc({
            route: '/odoo_studio_community/rename_field',
            params: {
    ...
    _restoreDefaultView: async function (view_id) {
        ...
        const result = await this._rpc({
            route: '/odoo_studio_community/restore_default_view',
            params: {
                view_id: view_id,
            },
        });
    ...
    _setDefaultValue: function (model_name, field_name, value) {
        ...
        return this._rpc({route: '/odoo_studio_community/set_default_value', params: params});
    },
    /**
     * @private
     * @param {String} model_name
    ...
    _setEmailAlias: function (model_name, value) {
        return this._rpc({
            route: '/odoo_studio_community/set_email_alias',
            params: {
                model_name: model_name,
                value: value,
            },
    ...
    */
    ...
    _onApprovalChange: async function(ev) {
        ...
        bus.trigger('toggle_snack_bar', 'saving');
        await this._rpc({
            route: '/odoo_studio_community/edit_approval',
            params: {
                model: this.model_name,
    ...
    _onNodeClicked: function (ev) {
        ...
        if (this.view_type === 'form' && node.tag === 'field') {
            ...
            if (!ev.data.isWowl && isX2Many && !_.contains(notEditableWidgets, attrs.widget)) {
                ...
                // create the subviews
                var message = $(QWeb.render('odoo_studio_community.X2ManyEdit'));
                var options = {
                ...
                $node.find('.o_odoo_studio_community_editX2Many').click(function (e) {
                    e.preventDefault();
                    e.stopPropagation();
