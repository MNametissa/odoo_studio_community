...
import * as formEditorRendererComponents from "@odoo_studio_community/client_action/view_editors/form/form_editor_renderer/form_editor_renderer_components";
import {
    cleanHooks,
    getActiveHook,
    getDroppedValues,
...
} from "@odoo_studio_community/client_action/view_editors/utils";
import { ChatterContainer, ChatterContainerHook } from "../chatter_container";
import { StudioHook } from "@odoo_studio_community/client_action/view_editors/components/studio_hook_component";
import { FieldStudio } from "@odoo_studio_community/client_action/view_editors/components/field_studio";
import { WidgetStudio } from "@odoo_studio_community/client_action/view_editors/components/widget_studio";
import { ViewButtonStudio } from "@odoo_studio_community/client_action/view_editors/components/view_button_studio";
...
const HOOK_CLASS_WHITELIST = [
    "o_odoo_studio_community_field_picture",
    "o_odoo_studio_community_field_html",
    "o_odoo_studio_community_field_many2many",
    "o_odoo_studio_community_field_one2many",
    "o_odoo_studio_community_field_tabs",
    "o_odoo_studio_community_field_columns",
    "o_odoo_studio_community_field_lines",
];
    ...
    setup() {
        ...
        useEffect(
            (rootEl, showInvisible) => {
                ...
                rootEl.classList.add("o_odoo_studio_community_form_view_editor");
                if (showInvisible) {
                    rootEl
                        .querySelectorAll(
                        ...
                        .forEach((el) => {
                            el.classList.add("o_odoo_studio_community_show_invisible");
                            el.classList.remove("o_invisible_modifier");
                        });
                } else {
                    rootEl
                        .querySelectorAll(
                            ":not(.o_FormRenderer_chatterContainer) .o_odoo_studio_community_show_invisible"
                        )
                        .forEach((el) => {
                            el.classList.remove("o_odoo_studio_community_show_invisible");
        ...
        const highlightNearestHook = (draggedEl, { x, y }) => {
            ...
            closestHookEl.classList.add("o_odoo_studio_community_nearest_hook");
            return true;
        };
