...
import { computeXpath, applyInvisible } from "@odoo_studio_community/client_action/view_editors/xml_utils";
        ...
        if (title) {
            if (
                ...
                if (h1 && h1.classList.contains("d-flex") && h1.classList.contains("flex-row")) {
                    avatarHook.setAttribute("class", `'oe_avatar ms-3 p-3 o_odoo_studio_community_avatar h4'`);
                    h1.append(avatarHook);
                } else {
                    avatarHook.setAttribute("class", `'oe_avatar ms-3 me-3 o_odoo_studio_community_avatar'`);
