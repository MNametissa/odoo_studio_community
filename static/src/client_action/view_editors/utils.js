...
export function cleanHooks(el) {
    for (const hookEl of el.querySelectorAll(".o_odoo_studio_community_nearest_hook")) {
        hookEl.classList.remove("o_odoo_studio_community_nearest_hook");
    }
}

export function getActiveHook(el) {
    return el.querySelector(".o_odoo_studio_community_nearest_hook");
}
...
export function getHooks(el) {
    return [...el.querySelectorAll(".o_odoo_studio_community_hook")];
}
...
export function useLegacyOnDropElement(ref, executeDrop) {
    ...
    const onLegacyDropped = (ev, ui) => {
        ...
        const isNew = $droppedEl[0].classList.contains("o_odoo_studio_community_component");
