...
import ReportEditorManager from "odoo_studio_community.ReportEditorManager";
...
class ReportEditorAdapter extends ComponentAdapter {
    ...
    async _readPaperFormat() {
        this.paperFormat = "A4";
        const result = await this.rpc("/odoo_studio_community/read_paperformat", {
    ...
    async _readWidgetsOptions() {
        this.widgetsOptions = await this.rpc("/odoo_studio_community/get_widgets_available_options", {
            context: this.user.context,
        });
    }
    /**
    ...
    async _getReportViews() {
        ...
        this.reportViews = await this.rpc("/odoo_studio_community/get_report_views", {
    ...
    async _editReport(values) {
        const result = await this.rpc("/odoo_studio_community/edit_report", {
...
ReportEditor.template = xml`<div class="o_odoo_studio_community_client_action"><ReportEditorAdapter /></div>`;
ReportEditor.components = { ReportEditorAdapter };
registry.category("actions").add("odoo_studio_community.report_editor", ReportEditor);
