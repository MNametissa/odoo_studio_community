...
import { MapNewViewDialog } from "@odoo_studio_community/client_action/editor/new_view_dialogs/map_new_view_dialog";
...
export class EditorAdapter extends ComponentAdapter {
    ...
    async _onNewView(data) {
        ...
        const activityAllowed = await this.rpc("/odoo_studio_community/activity_allowed", {
    ...
    async addViewType(action, viewType, args) {
        let viewAdded = await this.rpc("/odoo_studio_community/add_view_type", {
    ...
    async editAction(action, args) {
        ...
        const result = await this.rpc("/odoo_studio_community/edit_action", {
            action_type: action.type,
            action_id: action.id,
            args: args,
            context: this.user.context,
    ...
    _onRestoreDefaultView(data) {
        ...
        const confirm = async () => {
            ...
            return this.rpc("/odoo_studio_community/restore_default_view", {
...
registry.category("actions").add("odoo_studio_community.action_editor", EditorAdapter);
