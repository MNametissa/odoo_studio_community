...
import { ModelConfigurator } from "@odoo_studio_community/client_action/model_configurator/model_configurator";
import { BG_COLORS, COLORS, ICONS } from "@odoo_studio_community/utils";
...
class AppCreator extends Component {
    ...
    async createNewApp() {
        ...
        try {
            const result = await this.rpc({
                route: "/odoo_studio_community/create_new_app",
                params: {
    ...
    onKeydown(ev) {
        if (
            ...
            !(
                ev.target.classList &&
                ev.target.classList.contains("o_odoo_studio_community_app_creator_previous")
            )
        ) {
            ev.preventDefault();
            this.onNext();
...
AppCreator.template = "odoo_studio_community.AppCreator";
