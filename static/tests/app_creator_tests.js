...
import { AppCreatorWrapper } from "@odoo_studio_community/client_action/app_creator/app_creator";
import { IconCreator } from "@odoo_studio_community/client_action/icon_creator/icon_creator";
...
QUnit.module("Studio", (hooks) => {
    ...
    QUnit.test("app creator: standard flow with model creation", async (assert) => {
        ...
        const { state, target } = await createAppCreator({
            ...
            async rpc(route, params) {
                if (route === "/odoo_studio_community/create_new_app") {
        ...
        assert.containsNone(
            target,
            ".o_odoo_studio_community_app_creator_previous",
            "Previous button should not be rendered at step welcome"
        );
        assert.hasClass(
            target.querySelector(".o_odoo_studio_community_app_creator_next"),
            "is_ready",
            "Next button should be ready at step welcome"
        );

        // go to step: 'app'
        await click(target, ".o_odoo_studio_community_app_creator_next");

        assert.strictEqual(state.step, "app", "Current step should be app");
        assert.containsOnce(
            target,
            ".o_odoo_studio_community_icon_creator .o_odoo_studio_community_selectors",
        ...
        await click(target.getElementsByClassName("o_odoo_studio_community_selector")[0]);

        assert.containsOnce(target, ".o_odoo_studio_community_palette", "the first palette should be open");

        await triggerEvent(target, ".o_odoo_studio_community_palette", "mouseleave");

        assert.containsNone(
            target,
            ".o_odoo_studio_community_palette",
            "leaving palette with mouse should close it"
        );

        await click(target.querySelectorAll(".o_odoo_studio_community_selectors > .o_odoo_studio_community_selector")[0]);
        await click(target.querySelectorAll(".o_odoo_studio_community_selectors > .o_odoo_studio_community_selector")[1]);
        ...
        await click(target.querySelectorAll(".o_odoo_studio_community_palette div")[2]);
        await click(target.querySelectorAll(".o_odoo_studio_community_selectors > .o_odoo_studio_community_selector")[2]);
        await click(target.querySelectorAll(".o_odoo_studio_community_icons_library div")[43]);

        await triggerEvent(target, ".o_odoo_studio_community_icons_library", "mouseleave");
        ...
        assert.strictEqual(
            target.querySelectorAll(".o_odoo_studio_community_selector")[1].style.backgroundColor,
        ...
        assert.hasClass(
            target.querySelector(".o_odoo_studio_community_selector i"),
            "fa fa-heart",
        ...
        target.querySelector(".o_odoo_studio_community_upload input").dispatchEvent(new Event("change"));
        await nextTick();
        ...
        assert.strictEqual(
            target.querySelector(".o_odoo_studio_community_uploaded_image").style.backgroundImage,
        ...
        assert.hasClass(
            appNameInput,
            "o_odoo_studio_community_app_creator_field_warning",
        ...
        await click(target, ".o_odoo_studio_community_model_configurator_previous");
        await click(target, ".o_odoo_studio_community_app_creator_next");
        ...
        await click(target, ".o_odoo_studio_community_model_configurator_next");
    ...
    QUnit.test("app creator: has 'lines' options to auto-create a one2many", async (assert) => {
        ...
        assert.containsOnce(
            target,
            ".o_odoo_studio_community_model_configurator_option input[type='checkbox'][name='lines'][id='lines']"
        );
        assert.strictEqual(
        ...
        await click(
            target,
            ".o_odoo_studio_community_model_configurator_option input[type='checkbox'][name='lines']"
        );
        await click(target, ".o_odoo_studio_community_model_configurator_next");
    });
    ...
    QUnit.test("app creator: debug flow with existing model", async (assert) => {
        ...
        const { state, target } = await createAppCreator({
            ...
            async rpc(route, params) {
                ...
                switch (route) {
                    ...
                    case "/odoo_studio_community/create_new_app": {
                        assert.strictEqual(
                            params.model_id,
                            69,
        ...
        let buttonNext = target.querySelector("button.o_odoo_studio_community_app_creator_next");
        ...
        buttonNext = target.querySelector("button.o_odoo_studio_community_app_creator_next");

        // check the 'existing model' radio
        await click(target, 'input[name="model_choice"][value="existing"]');

        assert.doesNotHaveClass(
            target.querySelector(".o_odoo_studio_community_app_creator_model"),
            "o_odoo_studio_community_app_creator_field_warning"
        );
        ...
        assert.verifySteps([
            ...
            "/odoo_studio_community/create_new_app",
        ]);
    });
