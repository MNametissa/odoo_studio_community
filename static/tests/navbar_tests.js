...
import { StudioNavbar } from "@odoo_studio_community/client_action/navbar/navbar";
...
QUnit.module("Studio > Navbar", (hooks) => {
    ...
    QUnit.test("homemenu customizer rendering", async (assert) => {
        ...
        assert.containsOnce(target, ".o_odoo_studio_community_home_studio_menu");

        await click(target.querySelector(".o_odoo_studio_community_home_studio_menu .dropdown-toggle"));

        assert.containsOnce(target, ".o_odoo_studio_community_change_background");
        assert.strictEqual(
            target.querySelector(".o_odoo_studio_community_change_background input").accept,
            "image/*",
            "Input should now only accept images"
        );

        assert.containsOnce(target, ".o_odoo_studio_community_import");
        assert.containsOnce(target, ".o_odoo_studio_community_export");
    });
});
