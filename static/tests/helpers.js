...
import { systrayItem } from "@odoo_studio_community/systray_item/systray_item";
...
import { studioService } from "@odoo_studio_community/studio_service";
...
export async function openStudio(target, params = {}) {
    await click(target.querySelector(".o_main_navbar .o_odoo_studio_community_navbar_item a"));
    await legacyExtraNextTick();
    if (params.noEdit) {
        const studioTabViews = target.querySelector(".o_odoo_studio_community_menu_item a");
        await click(studioTabViews);
        const controlElm = target.querySelector(
            ".o_action_manager .o_odoo_studio_community_editor .o_odoo_studio_community_views"
        );
        if (!controlElm) {
    ...
    if (params.report) {
        const studioTabReport = target.querySelectorAll(".o_odoo_studio_community_menu_item a")[1];
        await click(studioTabReport);
        await legacyExtraNextTick();
        let controlElm = target.querySelector(
            ".o_action_manager .o_odoo_studio_community_editor .o_odoo_studio_community_report_kanban"
        );
        if (!controlElm) {
        ...
        controlElm = target.querySelector(
            ".o_action_manager .o_odoo_studio_community_editor .o_odoo_studio_community_report_editor_manager"
        );
        if (!controlElm) {
...
export async function leaveStudio(target) {
    await click(target.querySelector(".o_studio_navbar .o_odoo_studio_community_leave a"));
...
export function getReportServerData() {
    ...
    const views = {
        "ir.actions.report,false,kanban": `
            <kanban class="o_odoo_studio_community_report_kanban" js_class="studio_report_kanban">
