odoo.define('odoo_studio_community.ActionEditorActionTests', function (require) {
    "use strict";
    ...
    QUnit.module('Studio', {
        ...
        QUnit.test('add a gantt view', async function (assert) {
            ...
            const mockRPC = (route, args) => {
                if (route === '/odoo_studio_community/add_view_type') {
            ...
            await testUtils.dom.click($(target).find('.o_odoo_studio_community_view_type[data-type="gantt"] .o_odoo_studio_community_thumbnail'));
            await legacyExtraNextTick();

            assert.containsOnce($, '.o_odoo_studio_community_new_view_dialog',
                "there should be an opened dialog to select gantt attributes");
            assert.strictEqual($('.o_odoo_studio_community_new_view_dialog select[name="date_start"]').val(), 'start',
                "date start should be prefilled (mandatory)");
            assert.strictEqual($('.o_odoo_studio_community_new_view_dialog select[name="date_stop"]').val(), 'start',
        ...
        QUnit.test('disable the view from studio', async function (assert) {
            ...
            const mockRPC = (route, args) => {
                if (route === '/odoo_studio_community/edit_action') {
                    return true;
            ...
            await testUtils.dom.click(target.querySelector('.o_odoo_studio_community_menu_item a'));

            // make list view disable and form view only will be there in studio view
            await testUtils.dom.click($(target).find('div[data-type="list"] .o_odoo_studio_community_more'));
            await testUtils.dom.click($(target).find('div[data-type="list"] a[data-action="disable_view"]'));
            // reloadAction = false;
            assert.hasClass(
                $(target).find('div[data-type="list"]'),
                'o_odoo_studio_community_inactive',
                "list view should have become inactive");

            // make form view disable and it should prompt the alert dialog
            await testUtils.dom.click($(target).find('div[data-type="form"] .o_odoo_studio_community_more'));
        ...
        QUnit.test("active_id and active_ids present in context at reload", async function (assert) {
            ...
            await editInput(target, ".o_odoo_studio_community_sidebar #name", "new name");
            assert.verifySteps([
                "edit_action",
