odoo.define('odoo_studio_community.ViewEditorManager_tests', function (require) {
"use strict";
...
const { ListEditorRenderer } = require("@odoo_studio_community/client_action/view_editors/list/list_editor_renderer");
var testUtils = require('web.test_utils');
var { session } = require('@web/session');
const { RPCError } = require("@web/core/network/rpc_service");

var studioTestUtils = require('odoo_studio_community.testUtils');

const { patchWithCleanup, nextTick, makeDeferred } = require("@web/../tests/helpers/utils");
const { registerCleanup } = require("@web/../tests/helpers/cleanup");

const { openStudio, registerStudioDependencies } = require("@odoo_studio_community/../tests/helpers");
...
const { _KanbanEditorRecord } = require("@odoo_studio_community/client_action/view_editors/kanban/kanban_editor_record")

const { registry } = require("@web/core/registry");
const { SIDEBAR_SAFE_FIELDS } = require("@odoo_studio_community/legacy/js/views/sidebar_safe_fields");
...
let target;
let pyEnv;
QUnit.module('odoo_studio_community', {}, function () {
QUnit.module('ViewEditorManager', {
    ...
    QUnit.test('list editor sidebar', async function (assert) {
        ...
        assert.containsOnce(vem, '.o_odoo_studio_community_sidebar',
            "there should be a sidebar");
        assert.hasClass(vem.$('.o_odoo_studio_community_sidebar').find('.o_odoo_studio_community_new'),'active',
            "the Add tab should be active in list view");
        assert.strictEqual(vem.$('.o_odoo_studio_community_sidebar').find('.o_odoo_studio_community_field_type_container').length, 2,
            "there should be two sections in Add (new & existing fields");

        await testUtils.dom.click(vem.$('.o_odoo_studio_community_sidebar').find('.o_odoo_studio_community_view'));

        assert.hasClass(vem.$('.o_odoo_studio_community_sidebar').find('.o_odoo_studio_community_view'),'active',
            "the View tab should now be active");
        assert.hasClass(vem.$('.o_odoo_studio_community_sidebar').find('.o_odoo_studio_community_properties'),'disabled',
    ...
    QUnit.test('search existing fields into sidebar', async function (assert) {
        ...
        assert.containsN(vem,
            '.o_odoo_studio_community_field_type_container.o_odoo_studio_community_existing_fields div.o_odoo_studio_community_component', 12);

        const $input = vem.$('.o_odoo_studio_community_sidebar_search_input');

        $input.val("a");
        await testUtils.fields.triggerKeyup($input);
        assert.containsN(vem,
            '.o_odoo_studio_community_field_type_container.o_odoo_studio_community_existing_fields div.o_odoo_studio_community_component', 8);

        $input.val("ar");
        await testUtils.fields.triggerKeyup($input);
        assert.containsN(vem,
            '.o_odoo_studio_community_field_type_container.o_odoo_studio_community_existing_fields div.o_odoo_studio_community_component', 2);

        $input.val("art");
        await testUtils.fields.triggerKeyup($input);
        assert.containsOnce(vem,
            '.o_odoo_studio_community_field_type_container.o_odoo_studio_community_existing_fields div.o_odoo_studio_community_component');
    ...
    QUnit.test('empty list editor', async function (assert) {
        ...
        assert.containsOnce(vem, '.o_odoo_studio_community_list_view_editor',
            "there should be a list editor");
        assert.containsOnce(vem, '.o_odoo_studio_community_list_view_editor table thead th.o_odoo_studio_community_hook',
            "there should be one hook");
        assert.containsNone(vem, '.o_odoo_studio_community_list_view_editor [data-studio-xpath]',
            "there should be no node");
        var nbFields = _.size(this.data.coucou.fields);
        assert.strictEqual(vem.$('.o_odoo_studio_community_sidebar .o_odoo_studio_community_existing_fields').children().length, nbFields,
    ...
    QUnit.test('list editor', async function (assert) {
        ...
        assert.containsOnce(vem, '.o_odoo_studio_community_list_view_editor [data-studio-xpath]',
            "there should be one node");
        assert.containsN(vem, 'table thead th.o_odoo_studio_community_hook', 2,
            "there should be two hooks (before & after the field)");
        var nbFields = _.size(this.data.coucou.fields) - 1; // - display_name
        assert.strictEqual(vem.$('.o_odoo_studio_community_sidebar').find('.o_odoo_studio_community_existing_fields').children().length, nbFields,
    ...
    QUnit.test('optional field in list editor', async function (assert) {
        ...
        await testUtils.dom.click(vem.$('.o_odoo_studio_community_view_renderer [data-studio-xpath]'));
        assert.containsOnce(
            vem,
            '.o_odoo_studio_community_sidebar_optional_select',
    ...
    QUnit.test('new field should come with show as default value of optional', async function (assert) {
        ...
        const vem = await studioTestUtils.createViewEditorManager({
            ...
            mockRPC: function (route, args) {
                if (route === '/odoo_studio_community/edit_view') {
        ...
        await testUtils.dom.dragAndDrop(vem.$('.o_odoo_studio_community_new_fields .o_odoo_studio_community_field_char'), $('.o_odoo_studio_community_hook'));
    });
    ...
    QUnit.test('new field before a button_group', async function (assert) {
        ...
        await testUtils.dom.dragAndDrop(vem.$('.o_odoo_studio_community_new_fields .o_odoo_studio_community_field_char'),
            $('.o_odoo_studio_community_hook')[0]);
    });
    ...
    QUnit.test('new field after a button_group', async function (assert) {
        ...
        await testUtils.dom.dragAndDrop(vem.$('.o_odoo_studio_community_new_fields .o_odoo_studio_community_field_char'),
            $('.o_odoo_studio_community_hook')[2]);
    });
    ...
    QUnit.test('invisible field in list editor', async function (assert) {
        ...
        await testUtils.dom.click(vem.$('.o_odoo_studio_community_view'));
        await testUtils.dom.click(vem.$('#show_invisible'));
        assert.containsOnce(vem, "th[data-name='display_name'].o_odoo_studio_community_show_invisible");

        await testUtils.dom.click(vem.$("th[data-name='display_name'].o_odoo_studio_community_show_invisible"));
    ...
    QUnit.test('visible studio hooks in listview', async function (assert) {
        ...
        assert.ok(
            vem.$('th.o_odoo_studio_community_hook')[0].offsetWidth,
            "studio hooks should be visible in non-editable listview");

        // check the same with editable list 'bottom'
        await testUtils.dom.click(vem.$('.o_odoo_studio_community_sidebar .o_odoo_studio_community_view'));
    ...
    QUnit.test('sortby and orderby field in sidebar', async function (assert) {
        ...
        await testUtils.dom.click(document.querySelector(".o_odoo_studio_community_sidebar .o_odoo_studio_community_view"));
        assert.containsOnce(vem, '.o_odoo_studio_community_sidebar_content.o_display_view select#sort_field', 'Sortby select box should be exist in slidebar.');
    ...
    QUnit.test('editing selection field of list of form view', async function(assert) {
        ...
        const mockRPC = (route, args) => {
            if (route === '/odoo_studio_community/edit_field') {
            ...
            if (route === '/odoo_studio_community/get_default_value') {
                return Promise.resolve({});
            }
        };
        ...
        await testUtils.dom.click($(target).find('button.o_odoo_studio_community_editX2Many[data-type="list"]'));
        await legacyExtraNextTick();

        // add value to "toughness" selection field
        await testUtils.dom.click($(target).find('th[data-studio-xpath]'));
        await testUtils.dom.click($(target).find('.o_odoo_studio_community_edit_selection_values'));
        $('.modal .o_odoo_studio_community_selection_new_value input').val('Hardest');
        await testUtils.dom.click($('.modal .o_odoo_studio_community_selection_new_value button.o_odoo_studio_community_add_selection_value'));
        await testUtils.dom.click($('.modal.o_odoo_studio_community_field_modal footer .btn-primary'));
    });
    ...
    QUnit.test('deleting selection field value which is linked in other records', async function (assert) {
        ...
        await testUtils.dom.click(vem.$('.o_odoo_studio_community_edit_selection_values'));
        assert.containsN($('.modal'), ".o_odoo_studio_community_selection_editor > li", 3,
            "there should be 2 selection values");

        await testUtils.dom.click($('.modal .o_odoo_studio_community_selection_editor > li:eq(2) .o_odoo_studio_community_remove_selection_value'));
        assert.containsN($('.modal'), ".o_odoo_studio_community_selection_editor > li", 2,
            "there should be 2 selection values");

        await testUtils.dom.click($('.modal button:contains(Confirm)'));
        assert.containsN($(document), '.modal', 2,
            "should contain 2 modals");
        assert.strictEqual($('.modal .o_odoo_studio_community_preserve_space').text(),
    ...
    QUnit.test('invisible list editor', async function(assert) {
        ...
        assert.containsNone(vem, '.o_odoo_studio_community_list_view_editor [data-studio-xpath]',
            "there should be no node");
        assert.containsOnce(vem, 'table thead th.o_odoo_studio_community_hook',
            "there should be one hook");

        // click on show invisible
        await testUtils.dom.click(vem.$('.o_odoo_studio_community_sidebar').find('.o_odoo_studio_community_view'));
        await testUtils.dom.click(vem.$('.o_odoo_studio_community_sidebar').find('input#show_invisible'));
    ...
    QUnit.test('list editor with invisible element checkbox', async function(assert){
        ...
        await testUtils.dom.click(document.querySelector('.o_odoo_studio_community_sidebar input#show_invisible'));
        await testUtils.dom.click(document.querySelector('.o_odoo_studio_community_sidebar .o_odoo_studio_community_new'));
        await testUtils.dom.click(document.querySelector('.o_odoo_studio_community_sidebar .o_odoo_studio_community_view'));
    ...
    QUnit.test('list editor invisible to visible on field', async function (assert) {
        ...
        // disable invisible
        await testUtils.dom.click(vem.$('.o_odoo_studio_community_sidebar').find('input#invisible'));
    });
    ...
    QUnit.test('list editor field', async function (assert) {
        ...
        // click on the field
        await testUtils.dom.click(vem.$('.o_odoo_studio_community_list_view_editor [data-studio-xpath]'));

        assert.hasClass(vem.$('.o_odoo_studio_community_list_view_editor [data-studio-xpath]'),'o-web-studio-editor--element-clicked',
            "the column should have the clicked style");

        assert.hasClass(vem.$('.o_odoo_studio_community_sidebar').find('.o_odoo_studio_community_properties'),'active',
            "the Properties tab should now be active");
        assert.containsOnce(vem, '.o_odoo_studio_community_sidebar_content.o_display_field',
            "the sidebar should now display the field properties");
        assert.strictEqual(vem.$('.o_odoo_studio_community_sidebar').find('input[name="string"]').val(), "Display Name",
            "the label in sidebar should be Display Name");
        assert.strictEqual(vem.$('.o_odoo_studio_community_sidebar').find('select[name="widget"]').val(), "char",
    ...
    QUnit.test('List grouped should not be grouped', async function (assert) {
        ...
        assert.containsNone(target, '.o_odoo_studio_community_list_view_editor .o_list_table_grouped',
    ...
    QUnit.test('move a field in list', async function (assert) {
        ...
        assert.strictEqual(vem.$('.o_odoo_studio_community_list_view_editor th').text(), "Display NameA charM2O",
            "the columns should be in the correct order");

        // move the m2o at index 0
        await dragAndDrop(target.querySelector('.o_odoo_studio_community_list_view_editor th[data-studio-xpath="/tree[1]/field[3]"]'),
            target.querySelector('th.o_odoo_studio_community_hook'));

        assert.strictEqual(vem.$('.o_odoo_studio_community_list_view_editor th').text(), "M2ODisplay NameA char",
    ...
    QUnit.test('list editor field with aggregate function', async function (assert) {
        ...
        assert.containsNone(vem, '.o_odoo_studio_community_sidebar select[name="aggregate"]',
        ...
        assert.containsOnce(vem, '.o_odoo_studio_community_sidebar select[name="aggregate"]',
            "should have aggregate selection for integer type column");

        // select 'sum' aggregate function
        await testUtils.fields.editAndTrigger(vem.$('.o_odoo_studio_community_sidebar').find('select[name="aggregate"]'), 'sum', ['change']);
        ...
        await testUtils.fields.editAndTrigger(vem.$('.o_odoo_studio_community_sidebar').find('select[name="aggregate"]'), 'avg', ['change']);
        ...
        await testUtils.fields.editAndTrigger(vem.$('.o_odoo_studio_community_sidebar').find('select[name="aggregate"]'), '', ['change']);
    ...
    QUnit.test('List readonly attribute should not set force_save', async function (assert) {
        ...
        // select data cell
        await testUtils.dom.click(vem.$('.o_odoo_studio_community_view_renderer').find('[data-name="display_name"]'));
        // check readonly
        await testUtils.dom.click(vem.$('.o_odoo_studio_community_sidebar').find('input#readonly'));
    });
    ...
    QUnit.test('empty form editor', async function (assert) {
        ...
        assert.containsOnce(vem, '.o_odoo_studio_community_form_view_editor',
            "there should be a form editor");
        assert.containsNone(vem, '.o_odoo_studio_community_form_view_editor .o-web-studio-editor--element-clickable',
            "there should be no node");
        assert.containsNone(vem, '.o_odoo_studio_community_form_view_editor .o_odoo_studio_community_hook',
    ...
    QUnit.test('form editor', async function (assert) {
        ...
        assert.containsOnce(vem, '.o_odoo_studio_community_form_view_editor .o-web-studio-editor--element-clickable',
            "there should be one node");
        assert.containsOnce(vem, '.o_odoo_studio_community_form_view_editor .o_odoo_studio_community_hook',
            "there should be one hook");

        await testUtils.dom.click(vem.$('.o_odoo_studio_community_form_view_editor .o-web-studio-editor--element-clickable'));

        assert.hasClass(vem.$('.o_odoo_studio_community_sidebar').find('.o_odoo_studio_community_properties'),'active',
            "the Properties tab should now be active");
        assert.containsOnce(vem, '.o_odoo_studio_community_sidebar_content.o_display_field',
            "the sidebar should now display the field properties");
        assert.hasClass(vem.$('.o_odoo_studio_community_form_view_editor .o-web-studio-editor--element-clickable'),'o-web-studio-editor--element-clicked',
            "the column should have the clicked style");
        assert.strictEqual(vem.$('.o_odoo_studio_community_sidebar').find('select[name="widget"]').val(), "char",
            "the widget in sidebar should be set by default");
    ...
    QUnit.test('optional field not in form editor', async function (assert) {
        ...
        await testUtils.dom.click(vem.$('.o_odoo_studio_community_view_renderer .o_field_char'));
        assert.containsNone(
            vem,
            '.o_odoo_studio_community_sidebar_optional_select',
    ...
    QUnit.test('many2one field edition', async function (assert) {
        ...
        assert.containsOnce(vem, '.o_odoo_studio_community_sidebar_content.o_display_field',
            "the sidebar should now display the field properties");
        assert.containsOnce(vem, '.o_odoo_studio_community_sidebar select[name="widget"] option[value="selection"]',
    ...
    QUnit.test('image field edition (change size)', async function (assert) {
        ...
        assert.containsOnce(vem, '.o_odoo_studio_community_form_view_editor .o_field_image',
        ...
        // edit the image
        await testUtils.dom.click(vem.$('.o_odoo_studio_community_form_view_editor .o_field_image'));

        assert.containsOnce(vem, '.o_odoo_studio_community_sidebar_content.o_display_field select#option_size',
            "the sidebar should display dropdown to change image size");
        assert.strictEqual(vem.$('.o_odoo_studio_community_sidebar_content.o_display_field select#option_size option:selected').val(), "[0,90]",
            "the image size should be correctly selected");
        assert.hasClass(vem.$('.o_odoo_studio_community_form_view_editor .o_field_image'),'o-web-studio-editor--element-clicked',
            "image should have the clicked style");

        // change image size to large
        await testUtils.fields.editSelect(vem.$('.o_odoo_studio_community_sidebar_content.o_display_field select#option_size'), "[0,270]");

        assert.verifySteps(["image, width: undefined, height: 270, previewImage: undefined"], "the image should have been fetched again");
        assert.strictEqual(vem.$('.o_odoo_studio_community_sidebar_content.o_display_field select#option_size option:selected').val(), "[0,270]",
            "the image size should be correctly selected");
    });
    ...
    QUnit.test('signature field edition (change full_name)', async function (assert) {
        ...
        await testUtils.dom.dragAndDrop(vem.$('.o_odoo_studio_community_new_fields .o_odoo_studio_community_field_signature'), vem.$('.o_inner_group .o_odoo_studio_community_hook:first'));

        assert.containsOnce(vem, '.o_odoo_studio_community_form_view_editor .o_signature',
            "there should be one signature field");

        // edit the signature
        await testUtils.dom.click(vem.$('.o_odoo_studio_community_form_view_editor .o_signature'));

        assert.containsOnce(vem, '.o_odoo_studio_community_sidebar_content.o_display_field select#option_full_name',
            "the sidebar should display dropdown to change 'Auto-complete with' field");

        assert.strictEqual(vem.$('.o_odoo_studio_community_sidebar_content.o_display_field select#option_full_name option:selected').val(), "",
            "the auto complete field should be empty by default");


        // change auto complete field to 'display_name'
        await testUtils.fields.editSelect(vem.$('.o_odoo_studio_community_sidebar_content.o_display_field select#option_full_name'), "display_name");

        assert.strictEqual(vem.$('.o_odoo_studio_community_sidebar_content.o_display_field select#option_full_name option:selected').val(), "display_name",
            "the auto complete field should be correctly selected");

        // change auto complete field to 'm2o'
        await testUtils.fields.editSelect(vem.$('.o_odoo_studio_community_sidebar_content.o_display_field select#option_full_name'), "m2o");

        assert.strictEqual(vem.$('.o_odoo_studio_community_sidebar_content.o_display_field select#option_full_name option:selected').val(), "m2o",
            "the auto complete field should be correctly selected");
    });
    ...
    QUnit.test('change widget binary to image (check default size)', async function (assert) {
        ...
        assert.containsOnce(vem, '.o_odoo_studio_community_sidebar_content.o_display_field select#widget',
            "the sidebar should display dropdown to change widget");
        assert.hasClass(vem.$('.o_odoo_studio_community_form_view_editor .o-web-studio-editor--element-clickable'),'o-web-studio-editor--element-clicked',
            "binary field should have the clicked style");

        // change widget to image
        await testUtils.fields.editSelect(vem.$('.o_odoo_studio_community_sidebar_content.o_display_field select#widget'), 'image');
    });
    ...
    QUnit.test('integer field should come with 0 as default value', async function(assert) {
        ...
        await testUtils.dom.dragAndDrop(vem.$('.o_odoo_studio_community_new_fields .o_odoo_studio_community_field_integer'), $('.o_odoo_studio_community_hook'));
        await testUtils.nextTick();
    });
    ...
    QUnit.test('invisible form editor', async function (assert) {
        ...
        assert.containsNone(vem, '.o_odoo_studio_community_form_view_editor .o_field_widget');
        assert.containsOnce(vem, '.o_odoo_studio_community_form_view_editor .o-web-studio-editor--element-clickable',
            "the invisible node should not be editable (only the group has a node-id set)");
        assert.containsN(vem, '.o_odoo_studio_community_form_view_editor .o_odoo_studio_community_hook', 2,
        ...
        assert.containsN(vem, '.o_odoo_studio_community_form_view_editor .o_odoo_studio_community_show_invisible', 2,
            "there should be one visible nodes (the invisible ones)");
        assert.containsNone(vem, '.o_odoo_studio_community_form_view_editor .o_invisible_modifier',
            "there should be no invisible node");
        assert.containsN(vem, '.o_odoo_studio_community_form_view_editor .o_odoo_studio_community_hook', 3,
    ...
    QUnit.test('form editor - chatter edition', async function (assert) {
        ...
        var vem = await studioTestUtils.createViewEditorManager({
            ...
            arch: "<form>" +
                ...
                "</form>",
            mockRPC: function(route, args) {
                if (route === '/odoo_studio_community/get_email_alias') {
                    return Promise.resolve({email_alias: 'coucou'});
                }
            },
        });

        assert.containsOnce(vem, '.o_odoo_studio_community_form_view_editor .o_FormRenderer_chatterContainer',
            "there should be a chatter node");

        // click on the chatter
        await testUtils.dom.click(vem.$('.o_odoo_studio_community_form_view_editor .o_FormRenderer_chatterContainer .o_odoo_studio_community_overlay'));

        assert.hasClass(vem.$('.o_odoo_studio_community_sidebar .o_odoo_studio_community_properties'),'active',
            "the Properties tab should now be active");
        assert.containsOnce(vem, '.o_odoo_studio_community_sidebar_content.o_display_chatter',
            "the sidebar should now display the chatter properties");
        assert.hasClass(vem.$('.o_odoo_studio_community_form_view_editor .o_FormRenderer_chatterContainer'),'o-web-studio-editor--element-clicked',
            "the chatter should have the clicked style");
        assert.strictEqual(vem.$('.o_odoo_studio_community_sidebar input[name="email_alias"]').val(), "coucou",
    ...
    QUnit.test('fields without value and label (outside of groups) are shown in form', async function (assert) {
        ...
        assert.doesNotHaveClass(vem.$('.o_odoo_studio_community_form_view_editor [name="id"]'), 'o_odoo_studio_community_widget_empty',
            "non empty field in group should label should not be special");
        assert.doesNotHaveClass(vem.$('.o_odoo_studio_community_form_view_editor [name="m2o"]'), 'o_odoo_studio_community_widget_empty',
            "empty field in group should have without label should not be special");
        assert.hasClass(vem.$('.o_odoo_studio_community_form_view_editor [name="m2o"]'),'o_field_empty',
            "empty field in group should have without label should still have the normal empty class");
        assert.doesNotHaveClass(vem.$('.o_odoo_studio_community_form_view_editor [name="display_name"]'), 'o_odoo_studio_community_widget_empty',
            "non empty field without label should not be special");
        assert.hasClass(vem.$('.o_odoo_studio_community_form_view_editor [name="char_field"]'),'o_odoo_studio_community_widget_empty',
            "empty field without label should be special");
        assert.strictEqual(vem.$('.o_odoo_studio_community_form_view_editor [name="char_field"]').text(), "A char",
    ...
    QUnit.test('invisible group in form sheet', async function (assert) {
        ...
        assert.containsOnce(vem.$('.o_odoo_studio_community_sidebar'), 'input#invisible',
        ...
        assert.hasClass(vem.$(".o_odoo_studio_community_sidebar .o_odoo_studio_community_new"), "active");

        await click(vem.$el[0].querySelector(".o_inner_group.kikou2"));
        const $groupInput = vem.$('.o_odoo_studio_community_sidebar_content.o_display_group input[name="string"]');
    ...
    QUnit.test('correctly display hook in form sheet', async function (assert) {
        ...
        target.querySelectorAll(".o_form_sheet > div.o_odoo_studio_community_hook").forEach(hook => {
            const control = sheetHooksValues.shift();
            assert.deepEqual(control, { ...hook.dataset });
        });

        assert.containsN(vem, '.o_odoo_studio_community_form_view_editor .o_form_sheet > div.o_odoo_studio_community_hook', 3,
        ...
        target.querySelectorAll(".o_form_sheet .o_inner_group > div.o_odoo_studio_community_hook").forEach(hook => {
            const control = innerGroupsHooksValues.shift();
            assert.deepEqual(control, { ...hook.dataset });
        });

        assert.hasClass(vem.$('.o_odoo_studio_community_form_view_editor .o_form_sheet > div:eq(1)'),'o_odoo_studio_community_hook',
            "second div should be a hook");
        assert.hasClass(vem.$('.o_odoo_studio_community_form_view_editor .o_form_sheet > div:eq(3)'),'o_odoo_studio_community_hook',
            "fourth div should be a hook");
        assert.hasClass(vem.$('.o_odoo_studio_community_form_view_editor .o_form_sheet > div:eq(5)'),'o_odoo_studio_community_hook',
    ...
    QUnit.test('correctly display hook below group title', async function (assert) {
        ...
        assert.strictEqual(vem.$('.o_odoo_studio_community_form_view_editor .o_inner_group:eq(0) .o_odoo_studio_community_hook').length, 1,
            "there should be 1 hook");
        assert.hasClass(vem.$('.o_odoo_studio_community_form_view_editor .o_inner_group:eq(0) > div:eq(0)'),'o_odoo_studio_community_hook',
            "the first div should be a hook");

        // second group (with title, without content)
        assert.strictEqual(vem.$('.o_odoo_studio_community_form_view_editor .o_inner_group:eq(1) .o_odoo_studio_community_hook').length, 1,
            "there should be 1 hook");
        assert.strictEqual(vem.$('.o_odoo_studio_community_form_view_editor .o_inner_group:eq(1) > div:eq(0)').text(), "Kikou2",
            "the first div is the group title");
        assert.hasClass(vem.$('.o_odoo_studio_community_form_view_editor .o_inner_group:eq(1) > div:eq(1)'),'o_odoo_studio_community_hook',
            "The second div should be a hook");

        // third group (without title, with content)
        assert.strictEqual(vem.$('.o_odoo_studio_community_form_view_editor .o_inner_group:eq(2) .o_odoo_studio_community_hook').length, 2,
            "there should be 2 hooks");
        assert.hasClass(vem.$('.o_odoo_studio_community_form_view_editor .o_inner_group:eq(2) > div:eq(0)'),'o_odoo_studio_community_hook',
            "the first div should be a hook");
        assert.strictEqual(vem.$('.o_odoo_studio_community_form_view_editor .o_inner_group:eq(2) > div:eq(1)').text(), "M2O",
            "the second div is the field");
        assert.hasClass(vem.$('.o_odoo_studio_community_form_view_editor .o_inner_group:eq(2) > div:eq(1) > div:eq(2)'),'o_odoo_studio_community_hook',
            "the hook should be placed after the field");

        // last group (with title, with content)
        assert.strictEqual(vem.$('.o_odoo_studio_community_form_view_editor .o_inner_group:eq(3) .o_odoo_studio_community_hook').length, 2,
            "there should be 2 hooks");
        assert.strictEqual(vem.$('.o_odoo_studio_community_form_view_editor .o_inner_group:eq(3) > div:eq(0)').text(), "Kikou",
            "the first div is the group title");
        assert.hasClass(vem.$('.o_odoo_studio_community_form_view_editor .o_inner_group:eq(3) > div:eq(1)'),'o_odoo_studio_community_hook',
            "the second div should be a hook");
        assert.strictEqual(vem.$('.o_odoo_studio_community_form_view_editor .o_inner_group:eq(3) > div:eq(2)').text(), "ID",
            "the third div is the field");
        assert.hasClass(vem.$('.o_odoo_studio_community_form_view_editor .o_inner_group:eq(3) > div:eq(2) > div:eq(2)'),'o_odoo_studio_community_hook',
    ...
    QUnit.test('correctly display hook at the end of tabs -- empty group', async function(assert) {
        ...
        assert.strictEqual(
            vem.$('.o_odoo_studio_community_form_view_editor .o_notebook .tab-pane.active').children().last().attr('class'),
            'o_odoo_studio_community_hook',
    ...
    QUnit.test("notebook page hooks", async (assert) => {
        ...
        assert.containsOnce(target, ".o_notebook .tab-pane.active > .o_odoo_studio_community_hook");
        assert.deepEqual({
            ...target.querySelector(".o_notebook .tab-pane.active > .o_odoo_studio_community_hook").dataset
        }, {
            "position": "inside",
            "type": "page",
    ...
    QUnit.test('notebook edition', async function (assert) {
        ...
        assert.containsOnce(vem, '.o_odoo_studio_community_sidebar_content.o_display_page',
            "the sidebar should now display the page properties");
        var $pageInput = vem.$('.o_odoo_studio_community_sidebar_content.o_display_page input[name="string"]');
        assert.strictEqual($pageInput.val(), "Kikou", "the page name in sidebar should be set");
        assert.strictEqual(vem.$('.o_odoo_studio_community_sidebar_content.o_display_page .o_groups .o_field_many2manytags').length, 1,
    ...
    QUnit.test('invisible notebook page in form', async function (assert) {
        ...
        const $pageInput = vem.$('.o_odoo_studio_community_sidebar_content.o_display_page input[name="string"]');
    ...
    QUnit.test('label edition', async function (assert) {
        ...
        var $label = $(vem.$('.o_odoo_studio_community_form_view_editor label')[0]);
        ...
        assert.containsOnce(vem, '.o_odoo_studio_community_sidebar_content.o_display_label',
            "the sidebar should now display the label properties");
        var $labelInput = vem.$('.o_odoo_studio_community_sidebar_content.o_display_label input[name="string"]');
        ...
        var $fieldLabel = vem.$('.o_odoo_studio_community_form_view_editor label:contains("A char")');
    ...
    QUnit.test('add a statusbar', async function (assert) {
        ...
        var $statusbar = vem.$('.o_odoo_studio_community_form_view_editor .o_odoo_studio_community_statusbar_hook');
        assert.deepEqual($statusbar.length, 1, "there should be a hook to add a statusbar");
        await testUtils.dom.click($statusbar);

        assert.deepEqual($('.o_odoo_studio_community_field_modal').length, 1,
            "a modal should be open to create the new selection field");
        assert.deepEqual($('.o_odoo_studio_community_selection_editor li').length, 3,
    ...
    QUnit.test('move a field in form', async function (assert) {
        ...
        assert.strictEqual(vem.$('.o_odoo_studio_community_form_view_editor .o_form_sheet').text(), "Display NameA charM2O",
            "the moved field should be the first column");

        // Don't be bothered by transition effects
        vem.el.querySelectorAll(".o_odoo_studio_community_hook_separator").forEach(sep => {
            sep.style.setProperty("transition", "none", "important");
        })
        // move m2o before display_name
        await dragAndDrop(vem.$('.o_odoo_studio_community_form_view_editor .o-draggable:eq(2)')[0],
            vem.$('.o_inner_group .o_odoo_studio_community_hook:first')[0]);

        assert.strictEqual(vem.$('.o_odoo_studio_community_form_view_editor .o_form_sheet').text(), "M2ODisplay NameA char",
            "the moved field should be the first column");
    ...
    QUnit.test('form editor add avatar image', async function (assert) {
        ...
        assert.containsOnce(vem, '.oe_avatar.o_odoo_studio_community_avatar',
            "there should be the hook for avatar image");

        // Test with existing field.
        await testUtils.dom.click(vem.$('.oe_avatar.o_odoo_studio_community_avatar'));
        ...
        assert.containsNone(vem, '.oe_avatar.o_odoo_studio_community_avatar',
            "there should be the hook for avatar image");

        // Remove already added field from view to test new image field case.
        await testUtils.dom.click(vem.$('.oe_avatar'));
        await testUtils.dom.click(vem.$('.o_odoo_studio_community_sidebar .o_odoo_studio_community_remove'));
    ...
    QUnit.test('sidebar for a related field', async function (assert) {
        ...
        assert.containsOnce(vem.el, ".o_odoo_studio_community_sidebar .o_odoo_studio_community_properties.active");
    ...
    QUnit.test('modification of field appearing multiple times in view', async function (assert) {
        ...
        var $visibleElement = $(vem.$('.o_odoo_studio_community_form_view_editor .o_wrap_label.o-web-studio-editor--element-clickable')[0]);
        ...
        var $labelInput = vem.$('.o_odoo_studio_community_sidebar_content input[name="string"]');
    ...
    QUnit.test("supports multiple occurences of field", async (assert) => {
        ...
        assert.containsN(target, ".o_odoo_studio_community_form_view_editor .o_inner_group .o-web-studio-editor--element-clickable", 1);
        await click(target, ".o_odoo_studio_community_sidebar_header div[name='view']");
        await click(target, ".o_odoo_studio_community_sidebar #show_invisible");
        assert.containsN(target, ".o_odoo_studio_community_form_view_editor .o_inner_group .o-web-studio-editor--element-clickable", 2);

        await click(target.querySelectorAll(".o_odoo_studio_community_form_view_editor .o_inner_group .o-web-studio-editor--element-clickable")[0]);
        assert.strictEqual(target.querySelector(".o_odoo_studio_community_sidebar input[name='enable_sms']").checked, false); // Would be true if not present in node's options

        await click(target.querySelectorAll(".o_odoo_studio_community_form_view_editor .o_inner_group .o-web-studio-editor--element-clickable")[1]);
        assert.strictEqual(target.querySelector(".o_odoo_studio_community_sidebar input[name='invisible']").checked, true);
    });
    ...
    QUnit.test("display placeholders when editing view without records", async (assert) => {
        ...
        assert.hasClass(vem.$("[name=display_name]"), "o_odoo_studio_community_widget_empty");
    ...
    QUnit.test("button rainbowman Truish value in sidebar", async function (assert) {
        ...
        assert.strictEqual(vem.el.querySelector(".o_odoo_studio_community_sidebar #rainbow").checked, true);
    });
    ...
    QUnit.test("button rainbowman False value in sidebar", async function (assert) {
        ...
        await click(vem.el.querySelector("button.oe_stat_button[data-studio-xpath]"));
        assert.strictEqual(vem.el.querySelector(".o_odoo_studio_community_sidebar #rainbow").checked, false);
    });
    ...
    QUnit.test('Form readonly attribute should set force_save to True', async function (assert) {
        ...
        // select data cell
        await testUtils.dom.click(vem.$('.o_odoo_studio_community_view_renderer').find('[name="display_name"]'));
        // check readonly
        await testUtils.dom.click(vem.$('.o_odoo_studio_community_sidebar').find('input#readonly'));
    });
    ...
    QUnit.test('empty kanban editor', async function (assert) {
        ...
        assert.containsOnce(vem, '.o_odoo_studio_community_kanban_view_editor',
            "there should be a kanban editor");
        assert.containsNone(vem, '.o_odoo_studio_community_kanban_view_editor .o-web-studio-editor--element-clickable',
            "there should be no node");
        assert.containsNone(vem, '.o_odoo_studio_community_kanban_view_editor .o_odoo_studio_community_hook',
            "there should be no hook");
    ...
    QUnit.test('kanban editor', async function (assert) {
        ...
        assert.containsOnce(vem, '.o_odoo_studio_community_kanban_view_editor .o-web-studio-editor--element-clickable',
            "there should be one node");
        assert.hasClass(vem.$('.o_odoo_studio_community_kanban_view_editor .o-web-studio-editor--element-clickable'),'o_odoo_studio_community_widget_empty',
            "the empty node should have the empty class");
        assert.containsOnce(vem, '.o_odoo_studio_community_kanban_view_editor .o_odoo_studio_community_hook',
            "there should be one hook");
        assert.containsOnce(vem, '.o_kanban_record .o_odoo_studio_community_add_kanban_tags',
            "there should be the hook for tags");
        assert.containsOnce(vem, '.o_kanban_record .o_odoo_studio_community_add_dropdown',
            "there should be the hook for dropdown");
        assert.containsOnce(vem, '.o_kanban_record .o_odoo_studio_community_add_priority',
            "there should be the hook for priority");
        assert.containsOnce(vem, '.o_kanban_record .o_odoo_studio_community_add_kanban_image',
            "there should be the hook for image");

        await testUtils.dom.click(vem.$('.o_odoo_studio_community_kanban_view_editor .o-web-studio-editor--element-clickable'));

        assert.hasClass(vem.$('.o_odoo_studio_community_sidebar').find('.o_odoo_studio_community_properties'),'active',
            "the Properties tab should now be active");
        assert.containsOnce(vem, '.o_odoo_studio_community_sidebar_content.o_display_field',
            "the sidebar should now display the field properties");
        assert.hasClass(vem.$('.o_odoo_studio_community_kanban_view_editor .o-web-studio-editor--element-clickable'),'o-web-studio-editor--element-clicked',
            "the field should have the clicked style");
        assert.strictEqual(vem.$('.o_odoo_studio_community_sidebar').find('select[name="widget"]').val(), "char",
            "the widget in sidebar should be set by default");
        assert.strictEqual(vem.$('.o_odoo_studio_community_sidebar').find('select[name="display"]').val(), "false",
    ...
    QUnit.test('no studio hook after a conditional field in the arch', async function (assert) {
        ...
        assert.containsNone(target, ".o_kanban_record .o_odoo_studio_community_hook");
    });
    ...
    QUnit.test('prevent crash when accessing details of implementation', async function (assert) {
        ...
        await click(target, ".o_odoo_studio_community_sidebar .o_odoo_studio_community_view");
        await click(target, ".o_odoo_studio_community_sidebar .o_odoo_studio_community_xml_editor");
        await XMLEditorDef;
        assert.strictEqual(target.querySelector(".o_kanban_record").textContent, "Preview is not available");
    });
    ...
    QUnit.test('kanban editor with async widget', async function (assert) {
        ...
        assert.containsNone(target, '.o_odoo_studio_community_kanban_view_editor');
        fieldDef.resolve();

        const vem = await prom;

        assert.containsOnce(target, '.o_odoo_studio_community_kanban_view_editor');
    ...
    QUnit.test('changing tab should reset the selected node', async function(assert) {
        ...
        await testUtils.dom.click(vem.$(".o_odoo_studio_community_kanban_view_editor span.o-web-studio-editor--element-clickable[data-field-name='display_name']"));
        assert.hasClass(vem.$('.o_odoo_studio_community_widget_empty.o-web-studio-editor--element-clickable'), 'o-web-studio-editor--element-clicked',
            "the field should have the clicked style");
    ...
    QUnit.test('kanban editor show invisible elements', async function(assert) {
        ...
        assert.containsN(vem, '.o_odoo_studio_community_kanban_view_editor .o-web-studio-editor--element-clickable', 3,
            "the 3 invisible fields should be visible now");
        assert.containsN(vem, '.o_odoo_studio_community_kanban_view_editor .o_odoo_studio_community_show_invisible.o-web-studio-editor--element-clickable', 3,
    ...
    QUnit.test('kanban editor add priority', async function (assert) {
        ...
        await testUtils.dom.click(vem.$('.o_kanban_record .o_odoo_studio_community_add_priority'));
        ...
        assert.containsNone(vem, '.o_kanban_record .o_odoo_studio_community_add_priority',
    ...
    QUnit.test('kanban editor no avatar button if already in arch', async function (assert) {
        ...
        assert.containsNone(vem, '.o_odoo_studio_community_add_kanban_image',
    ...
    QUnit.test('kanban editor add and remove image', async function (assert) {
        ...
        await testUtils.dom.click(vem.$('.o_kanban_record .o_odoo_studio_community_add_kanban_image'));
    ...
    QUnit.test('kanban editor with widget', async function (assert) {
        ...
        assert.strictEqual(vem.$('.o_odoo_studio_community_sidebar').find('select[name="widget"]').val(), "email",
    ...
    QUnit.test('grouped kanban editor', async function (assert) {
        ...
        assert.hasClass(vem.$('.o_odoo_studio_community_kanban_view_editor '),'o_kanban_grouped',
    ...
    QUnit.test('grouped kanban editor with record', async function (assert) {
        ...
        assert.containsOnce(vem, '.o_odoo_studio_community_kanban_view_editor .o-web-studio-editor--element-clickable',
            "there should be one node");
        assert.doesNotHaveClass(vem.$('.o_odoo_studio_community_kanban_view_editor .o-web-studio-editor--element-clickable'), 'o_odoo_studio_community_widget_empty',
    ...
    QUnit.test("specific dropdown arch with distinct parts in a '<a />' tag", async (assert) => {
        ...
        assert.containsOnce(target, ".o_odoo_studio_community_properties.active");
        assert.strictEqual(target.querySelector(".o_odoo_studio_community_sidebar_content").textContent.trim(), "");
    });
    ...
    QUnit.test('kanban editor remove "Set Cover Image" from dropdown menu', async function (assert) {
        ...
        var vem = await studioTestUtils.createViewEditorManager({
            ...
            arch: arch,
            mockRPC: function (route, args) {
                if (route === '/odoo_studio_community/edit_view' && args.operations[0].type === "remove") {
                    assert.deepEqual(args.operations[0], {
                        target: {
        ...
        await testUtils.dom.click(vem.$(".o_display_div .o_odoo_studio_community_sidebar_checkbox input"));
    });
    ...
    QUnit.test('kanban editor add "Set Cover Image" option in dropdown menu', async function (assert) {
        ...
        await testUtils.dom.click(vem.$(".o_kanban_record .o_dropdown_kanban"));
        assert.hasAttrValue(vem.$('.o_odoo_studio_community_sidebar input[name="set_cover"]'), 'checked', undefined,
            "Option to set cover should not be enabled");
        await testUtils.dom.click(vem.$('.o_odoo_studio_community_sidebar input[name="set_cover"]'));
    ...
    QUnit.test("drag and drop a new field", async (assert) => {
        ...
        await click(target, ".o_odoo_studio_community_sidebar .o_odoo_studio_community_sidebar_header [name='new']");

        await testUtils.dom.dragAndDrop(
            $(target.querySelector(".o_odoo_studio_community_sidebar_content .o_odoo_studio_community_component")),
            $(target.querySelector(".o_odoo_studio_community_kanban_view_editor .o_odoo_studio_community_hook"))
        );
    ...
    QUnit.test("add tags with no many2many field", async (assert) => {
        ...
        await click(target, ".o_odoo_studio_community_add_kanban_tags")
    ...
    QUnit.test("add tags", async (assert) => {
        ...
        assert.verifySteps(["edit_view"]);
        assert.containsNone(target,".o_odoo_studio_community_add_kanban_tags");
    });
    ...
    QUnit.test("add dropdown", async (assert) => {
        ...
        await click(target, ".o_odoo_studio_community_add_dropdown");
        await click(target.querySelector(".modal .modal-footer .btn-primary"));
        assert.verifySteps(["edit_view"]);
        assert.containsNone(target, ".o_odoo_studio_community_add_dropdown");
    });
    ...
    QUnit.test("toggle bold attribute", async (assert) => {
        ...
        assert.containsOnce(target, ".o_odoo_studio_community_properties.active");
        assert.hasAttrValue(
            target.querySelector(".o_odoo_studio_community_sidebar input#bold"),
            "checked",
            undefined,
            "show invisible checkbox is not checked"
        );

        await click(target, ".o_odoo_studio_community_sidebar input#bold");
        assert.verifySteps(["edit_view"]);
    ...
    QUnit.test("disallow using activeFields for the kanban priority", async function (assert) {
        ...
        await testUtils.dom.click(vem.$(".o_odoo_studio_community_add_priority"));

        assert.containsOnce(document.body, ".o_odoo_studio_community_kanban_helper");

        assert.deepEqual(
            Array.prototype.map.call(
                document.querySelectorAll(".o_odoo_studio_community_kanban_helper select option"),
                (e) => e.value
            ),
            ["", "priority"]
        );
    ...
    QUnit.test('empty search editor', async function (assert) {
        ...
        assert.containsOnce(vem, '.o_odoo_studio_community_search_view_editor',
            "there should be a search editor");
        assert.containsOnce(vem, '.o_odoo_studio_community_search_autocompletion_fields.table tbody tr.o_odoo_studio_community_hook',
            "there should be one hook in the autocompletion fields");
        assert.containsOnce(vem, '.o_odoo_studio_community_search_filters.table tbody tr.o_odoo_studio_community_hook',
            "there should be one hook in the filters");
        assert.containsOnce(vem, '.o_odoo_studio_community_search_group_by.table tbody tr.o_odoo_studio_community_hook',
            "there should be one hook in the group by");
        assert.containsNone(vem, '.o_odoo_studio_community_search_view_editor [data-node-id]',
            "there should be no node");
    });
    ...
    QUnit.test('empty search editor: drag a groupby', async function (assert) {
        ...
        await testUtils.nextTick();
        await testUtils.dom.dragAndDrop(
            vem.$('.o_odoo_studio_community_existing_fields > .ui-draggable[title="Last Modified on"]'),
            vem.$('.o_odoo_studio_community_search_group_by')
        );
        await testUtils.nextTick();

        assert.containsOnce(vem, ".o_odoo_studio_community_search_group_by.table [data-node-id]");
        assert.strictEqual(vem.$(".o_odoo_studio_community_search_group_by.table [data-node-id]").text().trim(), "Last Updated on");
    });
    ...
    QUnit.test('search editor', async function (assert) {
        ...
        await testUtils.dom.dragAndDrop(vem.$('.o_odoo_studio_community_existing_fields > .ui-draggable[title="Display Name"]'), $('.o_odoo_studio_community_search_autocompletion_fields .o_odoo_studio_community_hook:first'));

        assert.strictEqual(vem.view_type, 'search',
            "view type should be search");
        assert.containsOnce(vem, '.o_odoo_studio_community_search_view_editor',
            "there should be a search editor");
        assert.containsN(vem, '.o_odoo_studio_community_search_autocompletion_fields.table tbody tr.o_odoo_studio_community_hook', 2,
            "there should be two hooks in the autocompletion fields");
        assert.containsN(vem, '.o_odoo_studio_community_search_filters.table tbody tr.o_odoo_studio_community_hook', 3,
            "there should be three hook in the filters");
        assert.containsN(vem, '.o_odoo_studio_community_search_group_by.table tbody tr.o_odoo_studio_community_hook', 2,
            "there should be two hooks in the group by");
        assert.containsOnce(vem, '.o_odoo_studio_community_search_autocompletion_fields.table [data-node-id]',
            "there should be 1 node in the autocompletion fields");
        assert.containsN(vem, '.o_odoo_studio_community_search_filters.table [data-node-id]', 2,
            "there should be 2 nodes in the filters");
        assert.containsOnce(vem, '.o_odoo_studio_community_search_group_by.table [data-node-id]',
            "there should be 1 nodes in the group by");
        assert.containsN(vem, '.o_odoo_studio_community_search_view_editor [data-node-id]', 4,
            "there should be 4 nodes");

        // edit the autocompletion field
        await testUtils.dom.click($('.o_odoo_studio_community_search_view_editor .o_odoo_studio_community_search_autocompletion_container [data-node-id]'));
        ...
        assert.containsOnce(vem, '.o_odoo_studio_community_sidebar_content.o_display_field',
            "the sidebar should now display the field properties");
        assert.hasClass(vem.$('.o_odoo_studio_community_search_view_editor .o_odoo_studio_community_search_autocompletion_container [data-node-id]'),'o_odoo_studio_community_clicked',
            "the field should have the clicked style");
        assert.strictEqual(vem.$('.o_odoo_studio_community_sidebar').find('input[name="string"]').val(), "Display Name",
            "the field should have the label Display Name in the sidebar");
    ...
    QUnit.test('delete a field', async function (assert) {
        ...
        // edit the autocompletion field
        await testUtils.dom.click(vem.$('.o_odoo_studio_community_search_autocompletion_container [data-node-id]'));
        await testUtils.dom.click(vem.$('.o_odoo_studio_community_sidebar .o_odoo_studio_community_remove'));
        await testUtils.dom.click($('.modal .btn-primary'));
    ...
    QUnit.test('indicate that regular stored field(except date/datetime) can not be dropped in "Filters" section', async function (assert) {
        ...
        await testUtils.dom.dragAndDrop(vem.$('.o_odoo_studio_community_existing_fields > .ui-draggable[title="Display Name"]'), $('.o_odoo_studio_community_search_filters .o_odoo_studio_community_hook:first'), {disableDrop: true});

        assert.hasClass(vem.$('.o_odoo_studio_community_search_filters'), 'text-muted',
            "filter section should be muted");
        assert.doesNotHaveClass(vem.$('.o_odoo_studio_community_search_group_by'), 'text-muted',
            "groupby section should not be muted");
        assert.doesNotHaveClass(vem.$('.o_odoo_studio_community_search_autocompletion_fields'), 'text-muted',
    ...
    QUnit.test('indicate that ungroupable field can not be dropped in "Filters" and "Group by" sections', async function (assert) {
        ...
        await testUtils.dom.dragAndDrop(vem.$('.o_odoo_studio_community_existing_fields > .o_odoo_studio_community_field_integer:first'), $('.o_odoo_studio_community_search_group_by .o_odoo_studio_community_hook:first'), {disableDrop: true});

        assert.hasClass(vem.$('.o_odoo_studio_community_search_group_by'), 'text-muted',
    ...
    QUnit.test('many2many field can be dropped in "Group by" sections', async function (assert) {
        ...
        assert.containsN(vem, '.o_odoo_studio_community_search_sub_item .o_odoo_studio_community_search_group_by [data-node-id]', 1,
            'should have 1 group inside groupby dropdown');

        // try to add many2many field in groupby
        await testUtils.dom.dragAndDrop(vem.$('.o_odoo_studio_community_existing_fields > .o_odoo_studio_community_field_many2many:first'), $('.o_odoo_studio_community_search_group_by .o_odoo_studio_community_hook:first'));
        assert.containsN(vem, '.o_odoo_studio_community_search_sub_item .o_odoo_studio_community_search_group_by [data-node-id]', 2,
    ...
    QUnit.test('indicate that separators can not be dropped in "Automcompletion Fields" and "Group by" sections', async function (assert) {
        ...
        await testUtils.dom.dragAndDrop(vem.$('.o_odoo_studio_community_new_components > .o_odoo_studio_community_filter_separator'), $('.o_odoo_studio_community_search_group_by .o_odoo_studio_community_hook:first'), {disableDrop: true});
        await testUtils.nextTick();
        assert.hasClass(vem.$('.o_odoo_studio_community_search_group_by'), 'text-muted',
            "groupby section should be muted");
        assert.hasClass(vem.$('.o_odoo_studio_community_search_autocompletion_fields'),'text-muted',
            "autocompletion_fields section should be muted");
        assert.doesNotHaveClass(vem.$('.o_odoo_studio_community_search_filters'), 'text-muted',
    ...
    QUnit.test('indicate that filter can not be dropped in "Automcompletion Fields" and "Group by" sections', async function (assert) {
        ...
        // try to add seperator in groupby
        await testUtils.dom.dragAndDrop(vem.$('.o_odoo_studio_community_new_components > .o_odoo_studio_community_filter'), $('.o_odoo_studio_community_search_group_by .o_odoo_studio_community_hook:first'), { disableDrop: true });
        await testUtils.nextTick();
        assert.hasClass(vem.$('.o_odoo_studio_community_search_group_by'), 'text-muted',
            "groupby section should be muted");
        assert.hasClass(vem.$('.o_odoo_studio_community_search_autocompletion_fields'), 'text-muted',
    ...
    QUnit.test('move a date/datetime field in search filter dropdown', async function (assert) {
        ...
        assert.containsN(vem, '.o_odoo_studio_community_search_sub_item .o_odoo_studio_community_search_filters.table tbody tr.o_odoo_studio_community_hook', 3,
            "there should be three hooks in the filters dropdown");
        // try to add a field other date/datetime in the filters section
        await testUtils.dom.dragAndDrop(vem.$('.o_odoo_studio_community_existing_fields .o_odoo_studio_community_field_selection'), vem.$('.o_odoo_studio_community_search_sub_item .o_odoo_studio_community_search_filters .o_odoo_studio_community_hook').eq(1));
        assert.containsN(vem, '.o_odoo_studio_community_search_sub_item .o_odoo_studio_community_search_filters [data-node-id]', 2,
            'should have two filters inside filters dropdown');

        // try to add a date field in the filters section
        await testUtils.dom.dragAndDrop(vem.$('.o_odoo_studio_community_existing_fields .o_odoo_studio_community_field_datetime[title="Start Date"]'), vem.$('.o_odoo_studio_community_search_sub_item .o_odoo_studio_community_search_filters .o_odoo_studio_community_hook').eq(1));
        assert.containsN(vem, '.o_odoo_studio_community_search_sub_item .o_odoo_studio_community_search_filters.table tbody tr.o_odoo_studio_community_hook', 4,
            "there should be four hooks in the filters dropdown");
        assert.containsN(vem, '.o_odoo_studio_community_search_sub_item .o_odoo_studio_community_search_filters [data-node-id]', 3,
    ...
    QUnit.test('empty pivot editor', async function (assert) {
        ...
        await testUtils.dom.click(vem.$('.o_odoo_studio_community_sidebar_header [name="view"]'));
    });
    ...
    QUnit.test('empty graph editor', async function (assert) {
        ...
        return concurrency.delay(0).then(function () {
            assert.containsOnce(vem, '.o_odoo_studio_community_view_renderer .o_graph_renderer');
            assert.containsOnce(vem, '.o_odoo_studio_community_view_renderer .o_graph_renderer .o_graph_canvas_container canvas',
    ...
    QUnit.test('switching chart types in graph editor', async function (assert) {
        ...
        assert.containsN(vem, '.o_odoo_studio_community_sidebar_select [name="type"] option', 3, 'Default type contain 3 option');
    ...
    QUnit.test('empty gantt editor', async function(assert) {
        ...
        assert.containsOnce(vem, '.o_odoo_studio_community_view_renderer .o_gantt_view',
            "there should be a gantt view");
        assert.containsOnce(vem, '.o_odoo_studio_community_sidebar_content.o_display_view select[name="precision_day"]',
            "it should be possible to edit the day precision");

        vem.$('.o_odoo_studio_community_sidebar_content.o_display_view select[name="precision_day"] option[value="hour:quarter"]').prop('selected', true).trigger('change');
        await testUtils.nextTick();
    ...
    QUnit.test('marker popup fields in editor sidebar', async function (assert) {
        ...
        assert.containsOnce(vem, '.o_odoo_studio_community_sidebar .o_map_popup_fields',
            'Should have marker popup fields');
        assert.containsN(vem, '.o_odoo_studio_community_sidebar .o_map_popup_fields .badge', 2,
        ...
        await testUtils.dom.click(vem.$('.o_odoo_studio_community_sidebar .o_map_popup_fields .badge:first .o_delete'));
        assert.containsOnce(vem, '.o_odoo_studio_community_sidebar .o_map_popup_fields .badge',
    ...
    QUnit.test('error during tree rendering: undo', async function (assert) {
        ...
        assert.strictEqual($('.o_odoo_studio_community_view_renderer').length, 1,
            "there should only be one renderer");
        assert.containsOnce(vem, '.o_odoo_studio_community_list_view_editor [data-studio-xpath]',
            "the view should be back as normal with 1 field");
        assert.containsOnce(vem, '.o_odoo_studio_community_sidebar_content.o_display_view',
    ...
    QUnit.test('add a monetary field without currency_id', async function (assert) {
        ...
        await testUtils.dom.dragAndDrop(vem.$('.o_odoo_studio_community_new_fields .o_odoo_studio_community_field_monetary'), vem.$('th.o_odoo_studio_community_hook').first());
        ...
        await testUtils.dom.dragAndDrop(vem.$('.o_odoo_studio_community_new_fields .o_odoo_studio_community_field_related'), vem.$('th.o_odoo_studio_community_hook').first());
    ...
    QUnit.test('add a monetary field with currency_id', async function (assert) {
        ...
        assert.containsOnce(vem, '.o_odoo_studio_community_list_view_editor [data-studio-xpath]',
            "there should be one node");
        await testUtils.dom.dragAndDrop(vem.$('.o_odoo_studio_community_new_fields .o_odoo_studio_community_field_monetary'), $('.o_odoo_studio_community_hook'));
    ...
    QUnit.test('add a related field', async function (assert) {
        ...
        await testUtils.dom.dragAndDrop(vem.$('.o_odoo_studio_community_new_fields .o_odoo_studio_community_field_related'), $('.o_odoo_studio_community_hook'));
    ...
    QUnit.test('add a one2many field', async function (assert) {
        ...
        await testUtils.dom.dragAndDrop(vem.$('.o_odoo_studio_community_new_fields .o_odoo_studio_community_field_one2many'), $('.o_odoo_studio_community_hook'));
        assert.strictEqual($('.modal').length, 1, "a modal should be displayed");
    ...
    QUnit.test('add a one2many lines field', async function (assert) {
        ...
        await testUtils.dom.dragAndDrop(vem.$('.o_odoo_studio_community_new_fields .o_odoo_studio_community_field_lines'), $('.o_odoo_studio_community_hook'));
    ...
    QUnit.test('add a many2many field', async function(assert) {
        ...
        await testUtils.dom.dragAndDrop(vem.$('.o_odoo_studio_community_new_fields .o_odoo_studio_community_field_many2many'), $('.o_odoo_studio_community_hook'));
        assert.strictEqual($('.modal').length, 1, "a modal should be displayed");
    ...
    QUnit.test('add a many2one field', async function (assert) {
        ...
        await testUtils.dom.dragAndDrop(vem.$('.o_odoo_studio_community_new_fields .o_odoo_studio_community_field_many2one'), $('.o_odoo_studio_community_hook'));
        assert.strictEqual($('.modal').length, 1, "a modal should be displayed");
    ...
    QUnit.test('switch mode after element removal', async function (assert) {
        ...
        assert.containsN(vem, '.o_odoo_studio_community_list_view_editor [data-studio-xpath]', 2,
            "there should be two nodes");


        await testUtils.dom.click(vem.$('.o_odoo_studio_community_list_view_editor [data-studio-xpath]:first'));
        ...
        assert.containsOnce(vem, '.o_odoo_studio_community_list_view_editor [data-studio-xpath]',
            "there should be one node");
        assert.containsNone(vem, '.o_odoo_studio_community_sidebar_content.o_display_field',
    ...
    QUnit.test('open XML editor in read-only', async function (assert) {
        ...
        assert.containsOnce(vem, '.o_odoo_studio_community_view_renderer .o_form_readonly.o_odoo_studio_community_form_view_editor',
            "the form editor should be displayed");

        // open the XML editor
        await testUtils.dom.click(vem.$('.o_odoo_studio_community_sidebar_header [name="view"]'));
        assert.containsOnce(vem, '.o_odoo_studio_community_sidebar .o_odoo_studio_community_xml_editor',
            "there should be a button to open the XML editor");
        await testUtils.dom.click(vem.$('.o_odoo_studio_community_sidebar .o_odoo_studio_community_xml_editor'));

        assert.strictEqual(vem.$('.o_odoo_studio_community_view_renderer .o_form_readonly:not(.o_odoo_studio_community_form_view_editor)').length, 1,
    ...
    QUnit.test('XML editor: reset operations stack', async function (assert) {
        ...
        var vem = await studioTestUtils.createViewEditorManager({
            ...
            mockRPC: function (route, args) {
                if (route === '/odoo_studio_community/edit_view') {
                    return getCurrentMockServer()._mockReturnView(arch, "coucou");
                } else if (route === '/odoo_studio_community/edit_view_arch') {
                    return getCurrentMockServer()._mockReturnView(arch, "coucou");
        ...
        assert.containsOnce(vem, '.o_odoo_studio_community_form_view_editor',
            "the form editor should be displayed");
        // do an operation
        await testUtils.dom.click(vem.$('.o_odoo_studio_community_form_view_editor .o_field_widget[name="display_name"]'));
        await testUtils.fields.editAndTrigger(vem.$('.o_odoo_studio_community_sidebar input[name="string"]'), 'Kikou', 'change');
    ...
    QUnit.test('new button in buttonbox', async function (assert) {
        ...
        await testUtils.dom.click(vem.$('.o_odoo_studio_community_form_view_editor .o_odoo_studio_community_button_hook'));

        assert.strictEqual($('.modal:visible').length, 1, "there should be one modal");
        assert.strictEqual($('.o_odoo_studio_community_new_button_dialog').length, 1,
            "there should be a modal to create a button in the buttonbox");
        assert.strictEqual($('.o_odoo_studio_community_new_button_dialog .o_field_many2one').length, 1,
            "there should be a many2one for the related field");

        $('.o_odoo_studio_community_new_button_dialog .o_field_many2one input').focus();
        await testUtils.fields.editAndTrigger($('.o_odoo_studio_community_new_button_dialog .o_field_many2one input'), 'test', ['keyup', 'focusout']);
    ...
    QUnit.test('new button in buttonbox with first element invisible', async function (assert) {
        ...
        assert.containsOnce(target, ".oe_button_box .o_odoo_studio_community_button_hook");
    ...
    QUnit.test('update sidebar after edition', async function (assert) {
        ...
        assert.strictEqual(vem.el.querySelector(".o_odoo_studio_community_sidebar [name=string]").value, "Display Name");
        vem.$('.o_odoo_studio_community_sidebar [name="string"]').focus();
        await testUtils.fields.editAndTrigger(vem.$('.o_odoo_studio_community_sidebar [name="string"]'), 'test', 'change');
        assert.containsOnce(vem.el, '.o_wrap_label.o-web-studio-editor--element-clicked[data-field-name=display_name]');
    ...
    QUnit.test('default value in sidebar', async function (assert) {
        ...
        await testUtils.dom.click(vem.$('[name="display_name"]').parent());
        assert.strictEqual(vem.$('.o_odoo_studio_community_sidebar_content.o_display_field input[data-type="default_value"]').val(), "yolo",
            "the sidebar should now display the field properties");

        await testUtils.dom.click(vem.$('[name="priority"]').parent());
        assert.strictEqual(vem.$('.o_odoo_studio_community_sidebar_content.o_display_field select[data-type="default_value"]').val(), "1",
            "the sidebar should now display the field properties");
    ...
    QUnit.test('default value for new field name', async function (assert) {
        ...
        await testUtils.dom.dragAndDrop(vem.$('.o_odoo_studio_community_new_fields .o_odoo_studio_community_field_char'), vem.$('.o_inner_group .o_odoo_studio_community_hook:first'));
        await testUtils.dom.dragAndDrop(vem.$('.o_odoo_studio_community_new_fields .o_odoo_studio_community_field_float'), vem.$('.o_inner_group .o_odoo_studio_community_hook:first'));
    ...
    QUnit.test('remove starting underscore from new field value', async function (assert) {
        ...
        const vem = await studioTestUtils.createViewEditorManager({
            ...
            mockRPC: function (route, args) {
                if (route === '/odoo_studio_community/edit_view') {
                    ...
                    return getCurrentMockServer()._mockReturnView(newArch, "coucou");
                } else if (route === '/odoo_studio_community/rename_field') {
        ...
        await testUtils.dom.dragAndDrop(vem.el.querySelector('.o_odoo_studio_community_new_fields .o_odoo_studio_community_field_char'), vem.$('.o_inner_group .o_odoo_studio_community_hook:first'));

        // rename the field
        await testUtils.fields.editAndTrigger(vem.el.querySelector('.o_odoo_studio_community_sidebar input[name="name"]'), '__new', ['change']);
    ...
    QUnit.test('notebook and group not drag and drop in a group', async function (assert) {
        ...
        await testUtils.dom.dragAndDrop(vem.$('.o_odoo_studio_community_field_type_container .o_odoo_studio_community_field_tabs'), $('.o_group .o_odoo_studio_community_hook'));
        assert.strictEqual(editViewCount, 0,
            "the notebook cannot be dropped inside a group");
        await testUtils.dom.dragAndDrop(vem.$('.o_odoo_studio_community_field_type_container .o_odoo_studio_community_field_columns'), $('.o_group .o_odoo_studio_community_hook'));
        assert.strictEqual(editViewCount, 0,
    ...
    QUnit.test('drop monetary field outside of group', async function (assert) {
        ...
        await testUtils.dom.dragAndDrop(
            vem.$('.o_odoo_studio_community_new_fields .o_odoo_studio_community_field_monetary'),
            $('.o_odoo_studio_community_hook'),
            { disableDrop: true }
        );
        assert.containsNone(vem, '.o_odoo_studio_community_nearest_hook', "There should be no highlighted hook");
    });
    ...
    QUnit.test('add a selection field in non debug', async function (assert) {
        ...
        await testUtils.dom.dragAndDrop(
            vem.$('.o_odoo_studio_community_new_fields .o_odoo_studio_community_field_selection'),
            vem.$('.o_odoo_studio_community_hook:first'));
        assert.containsOnce($, '.modal .o_odoo_studio_community_field_dialog_form',
            "a modal should be opened");
        assert.containsNone($, '.modal .o_odoo_studio_community_selection_editor',
            "there should be no selection editor");

        // saving selection with no values should show a warning and not save
        assert.containsNone($, '.modal .o_odoo_studio_community_selection_editor > li',
        ...
        await testUtils.fields.editAndTrigger($('.modal .o_odoo_studio_community_selection_new_value input'),
            'Value 1', [$.Event('keyup', { which: $.ui.keyCode.ENTER })]);
        assert.containsOnce($, '.modal .o_odoo_studio_community_selection_editor > li',
            "there should be 1 selection value");
        assert.containsOnce($, '.modal .o_odoo_studio_community_selection_editor > li span:contains(Value 1)',
            "the value should be correctly set");

        // edit the first value
        await testUtils.dom.click($('.modal .o_odoo_studio_community_selection_editor li:first .o_odoo_studio_community_edit_selection_value'));
        assert.containsOnce($, '.modal',
            "new modal to edit selection value should not open in non debug mode");
        assert.strictEqual($('.modal .o_odoo_studio_community_selection_editor li:first').find('.o_odoo_studio_community_selection_input').val(), "Value 1",
            "the value should be set in the input in li");

        await testUtils.fields.editAndTrigger($('.modal .o_odoo_studio_community_selection_editor li:first .o_odoo_studio_community_selection_input'),
            'Miramar', ['blur']);
        assert.containsOnce($, '.modal .o_odoo_studio_community_selection_editor > li:first span:contains(Miramar)',
    ...
    QUnit.test('add a selection field in debug', async function (assert) {
        ...
        await testUtils.dom.dragAndDrop(vem.$('.o_odoo_studio_community_new_fields .o_odoo_studio_community_field_selection'), $('.o_odoo_studio_community_hook:first'));
        assert.strictEqual($('.modal .o_odoo_studio_community_field_dialog_form').length, 1, "a modal should be opened");
        assert.strictEqual($('.modal .o_odoo_studio_community_selection_editor').length, 0, "there should be no selection editor");
        ...
        // add a new value (with ENTER)
        $('.modal .o_odoo_studio_community_selection_new_value input')
            .val('Value 1')
            .trigger($.Event('keyup', {which: $.ui.keyCode.ENTER}));
        await testUtils.nextTick();
        assert.strictEqual($('.modal .o_odoo_studio_community_selection_editor > li').length, 1, "there should be 1 selection value");
        assert.strictEqual($('.modal .o_odoo_studio_community_selection_editor > li span:contains(Value 1)').length, 1, "the value should be correctly set");

        // add a new value (with button 'fa-check' )
        $('.modal .o_odoo_studio_community_selection_new_value input').val('Value 2');
        await testUtils.dom.click($('.modal .o_odoo_studio_community_add_selection_value'));
        assert.strictEqual($('.modal .o_odoo_studio_community_selection_editor > li').length, 2, "there should be 2 selection values");
        ...
        assert.strictEqual($('.modal .o_odoo_studio_community_selection_editor > li:first span:contains(My Value)').length, 1, "the value should have been updated");

        // add a value and delete it
        $('.modal .o_odoo_studio_community_selection_new_value input').val('Value 3');
        await testUtils.dom.click($('.modal .o_odoo_studio_community_add_selection_value'));
        assert.strictEqual($('.modal .o_odoo_studio_community_selection_editor > li').length, 3, "there should be 3 selection values");
        ...
        await testUtils.dom.dragAndDrop(
            ...
            $('.modal .o_odoo_studio_community_selection_editor > li').first(),
            {position: 'top'});
        assert.strictEqual($('.modal .o_odoo_studio_community_selection_editor > li:first span:contains(Value 2)').length, 1, "the values should have been reordered");
    ...
    QUnit.test('add a selection field with widget priority', async function (assert) {
        ...
        await testUtils.dom.dragAndDrop(vem.$('.o_odoo_studio_community_new_fields .o_odoo_studio_community_field_priority'), $('.o_odoo_studio_community_hook'));
    ...
    QUnit.test('blockUI not removed just after rename', async function (assert) {
        ...
        await testUtils.dom.dragAndDrop(vem.$('.o_odoo_studio_community_new_fields .o_odoo_studio_community_field_char'), vem.$('.o_odoo_studio_community_hook:first'));
        await testUtils.nextTick();
        assert.strictEqual(vem.$('thead th[data-studio-xpath]').length, 2, "there should be two fields");

        // rename the field
        await testUtils.fields.editAndTrigger(vem.$('.o_odoo_studio_community_sidebar input[name="name"]'), 'new', ['change']);

        assert.verifySteps([
            "/web/dataset/call_kw/coucou/web_search_read",
            'block UI',
            '/odoo_studio_community/edit_view',
            "/web/dataset/call_kw/coucou/web_search_read",
            'unblock UI',
            '/odoo_studio_community/get_default_value',
            'block UI',
            '/odoo_studio_community/rename_field',
            '/odoo_studio_community/edit_view',
            "/web/dataset/call_kw/coucou/web_search_read",
            '/odoo_studio_community/get_default_value',
            'unblock UI',
    ...
    QUnit.test('click on the "More" Button', async function (assert) {
        ...
        await doAction(webClient, "studio.coucou_action");
        await openStudio(target);
        await testUtils.dom.click(target.querySelector(".o_odoo_studio_community_view"));

        assert.containsOnce(target, '.o_odoo_studio_community_sidebar .o_odoo_studio_community_parameters',
            "there should be the button to go to the ir.ui.view form");
        await testUtils.dom.click(target.querySelector('.o_odoo_studio_community_sidebar .o_odoo_studio_community_parameters'));
        await legacyExtraNextTick();
    ...
    QUnit.test("calendar editor: constrain available periods to the scale attribute", async function(assert) {
        ...
        const availableModesOptions = vem.el.querySelectorAll(".o_odoo_studio_community_sidebar select[name='mode'] option");
        assert.deepEqual(
    ...
    QUnit.test('edit one2many form view (2 level) and check that the correct model is passed', async function (assert) {
        ...
        await testUtils.dom.click($(target).find('.o_odoo_studio_community_form_view_editor .o_field_one2many'));
        await testUtils.dom.click($(target).find('.o_odoo_studio_community_form_view_editor .o_field_one2many .o_odoo_studio_community_editX2Many[data-type="form"]'));
        await legacyExtraNextTick();
    ...
    QUnit.test('disable creation(no_create options) in many2many_tags widget', async function (assert) {
        ...
        await testUtils.dom.click($(target).find('.o_odoo_studio_community_view_renderer .o_field_many2many_tags'));
        assert.containsOnce(target, '.o_odoo_studio_community_sidebar #option_no_create',
            "should have no_create option for m2m field");
        assert.notOk($(target).find('.o_odoo_studio_community_sidebar #option_no_create').is(':checked'),
            'by default the no_create option should be false');

        await testUtils.dom.click($(target).find('.o_odoo_studio_community_sidebar #option_no_create'));
    });
    ...
    QUnit.test('disable creation(no_create options) in many2many_tags_avatar widget', async function (assert) {
        ...
        await testUtils.dom.click(vem.$('.o_odoo_studio_community_view_renderer .o_field_many2many_tags_avatar'));
        assert.containsOnce(vem, '.o_odoo_studio_community_sidebar #option_no_create',
            "should have no_create option for many2many_tags_avatar widget");
        assert.notOk(vem.$('.o_odoo_studio_community_sidebar #option_no_create').is(':checked'),
            'by default the no_create option should be false');

        await testUtils.dom.click(vem.$('.o_odoo_studio_community_sidebar #option_no_create'));
    ...
    QUnit.test('disable creation(no_create options) in many2many_avatar_user and many2many_avatar_employee widget', async function (assert) {
        ...
        await testUtils.dom.click($(target).find('.o_odoo_studio_community_view_renderer .o_field_many2many_avatar_user[name="m2m_users"]'));
        assert.containsOnce(target, '.o_odoo_studio_community_sidebar #option_no_create',
    ...
    QUnit.test('display one2many without inline views', async function (assert) {
        ...
        const mockRPC = (route, args) => {
            if (route === "/odoo_studio_community/create_inline_view") {
        ...
        await testUtils.dom.click($(target).find('.o_odoo_studio_community_view_renderer .o_field_one2many'));
        await testUtils.dom.click(
            $(target).find('.o_odoo_studio_community_view_renderer .o_field_one2many .o_odoo_studio_community_editX2Many[data-type="list"]'));
        await legacyExtraNextTick();
    });
    ...
    QUnit.test('edit one2many list view', async function (assert) {
        ...
        await testUtils.dom.click($(target).find('.o_odoo_studio_community_view_renderer .o_field_one2many'));
        const blockOverlayZindex = target.querySelector('.o_odoo_studio_community_view_renderer .o_field_one2many .o-web-studio-edit-x2manys-buttons').style['z-index'];
        assert.strictEqual(blockOverlayZindex, '1000',
            "z-index of blockOverlay should be 1000");
        assert.verifySteps(['coucou']);

        await testUtils.dom.click($($(target).find('.o_odoo_studio_community_view_renderer .o_field_one2many .o_odoo_studio_community_editX2Many')[0]));
        await legacyExtraNextTick();
        assert.containsOnce(target, '.o_odoo_studio_community_view_renderer thead tr [data-studio-xpath]',
            "there should be 1 nodes in the x2m editor.");

        await testUtils.dom.dragAndDrop($(target).find('.o_odoo_studio_community_existing_fields .o_odoo_studio_community_field_many2one')[0], $('.o_odoo_studio_community_hook'));
        await testUtils.nextTick();

        assert.containsN(target, '.o_odoo_studio_community_view_renderer thead tr [data-studio-xpath]', 2,
            "there should be 2 nodes after the drag and drop.");

        // click on a field in the x2m list view
        await testUtils.dom.click($(target).find('.o_odoo_studio_community_view_renderer [data-studio-xpath]:first'));
        await legacyExtraNextTick();
        ...
        await testUtils.dom.click($(target).find('.o_odoo_studio_community_sidebar .o_odoo_studio_community_parameters'));
    });
    ...
    QUnit.test('edit one2many list view with widget fieldDependencies and some records', async function (assert) {
        ...
        assert.containsOnce(target, ".o_odoo_studio_community_form_view_editor");
        await click(target.querySelector(".o_field_one2many"));
        await click(target.querySelector(".o_field_one2many .o_odoo_studio_community_editX2Many"));
        await legacyExtraNextTick();
        assert.verifySteps([
            "fields_get",
            `widget Dependency: {"type":"char","string":"Dependency from fields_get"} : the meters`,
        ])
        assert.containsOnce(target, ".o_odoo_studio_community_list_view_editor");
    ...
    QUnit.test("entering x2many with view widget", async (assert) => {
        ...
        await click(target, ".o_odoo_studio_community_view_renderer .o_field_one2many");
        await click(target, ".o_odoo_studio_community_view_renderer .o_field_one2many .o_odoo_studio_community_editX2Many[data-type='list']");
        await legacyExtraNextTick();
        assert.containsOnce(target, ".o_odoo_studio_community_list_view_editor");
        assert.containsOnce(target, ".myWidget");
    });
    ...
    patchWithCleanup(MockServer.prototype, {
        ...
        const mockRPC = (route, args) => {
            if (route === "/odoo_studio_community/chatter_allowed") {
                return true;
            }
        ...
        assert.containsOnce(target, '.o_odoo_studio_community_add_chatter',
            "should be possible to add a chatter");

        await testUtils.dom.click($(target).find('.o_odoo_studio_community_view_renderer .o_field_one2many'));
        await legacyExtraNextTick();
        await testUtils.dom.click(
            $(target).find('.o_odoo_studio_community_view_renderer .o_field_one2many .o_odoo_studio_community_editX2Many[data-type="form"]'));
        await legacyExtraNextTick();
        assert.containsNone(target, '.o_odoo_studio_community_add_chatter',
        ...
        await testUtils.dom.dragAndDrop(
            $(target).find('.o_odoo_studio_community_new_fields .o_odoo_studio_community_field_char'),
            $(target).find('.o_inner_group .o_odoo_studio_community_hook:first')
        );

        // add a new button
        await testUtils.dom.click($(target).find('.o_odoo_studio_community_form_view_editor .o_odoo_studio_community_button_hook'));
        assert.strictEqual($('.modal .o_odoo_studio_community_new_button_dialog').length, 1,
            "there should be an opened modal to add a button");
        await testUtils.dom.click($('.modal .o_odoo_studio_community_new_button_dialog .js_many2one_field input'));
    });
    ...
    QUnit.test('edit one2many list view that uses parent key [REQUIRE FOCUS]', async function (assert) {
        ...
        await testUtils.dom.click($(target).find('.o_odoo_studio_community_form_view_editor .o_field_one2many .o_odoo_studio_community_editX2Many[data-type="form"]'));
        await legacyExtraNextTick();
        assert.strictEqual($(target).find('.o_odoo_studio_community_form_view_editor .o_field_widget[name="m2o"]').text(), "jacques",
            "the x2m form view should be correctly rendered");
        await testUtils.dom.click($(target).find('.o_odoo_studio_community_form_view_editor .o_field_widget[name="m2o"]'));

        // open the domain editor
        assert.strictEqual($('.modal .o_domain_selector').length, 0,
            "the domain selector should not be opened");
        $(target).find('.o_odoo_studio_community_sidebar_content input[name="domain"]').trigger('focus');
        await testUtils.nextTick();
    ...
    QUnit.test('move a field in one2many list', async function (assert) {
        ...
        // edit the x2m form view
        await testUtils.dom.click($(target).find('.o_odoo_studio_community_form_view_editor .o_field_one2many'));
        await testUtils.dom.click($(target).find('.o_odoo_studio_community_form_view_editor .o_field_one2many .o_odoo_studio_community_editX2Many[data-type="list"]'));
        await legacyExtraNextTick();

        assert.strictEqual($(target).find('.o_odoo_studio_community_list_view_editor th').text(), "M2Ocoucou",
            "the columns should be in the correct order");

        // move coucou at index 0
        await testUtils.dom.dragAndDrop($(target).find('.o_odoo_studio_community_list_view_editor th:contains(coucou)'),
            $(target).find('th.o_odoo_studio_community_hook:first'));
    });
    ...
    QUnit.test('notebook and group drag and drop after a group', async function (assert) {
        ...
        var $afterGroupHook = vem.$('.o_form_sheet > .o_odoo_studio_community_hook');
        await testUtils.dom.dragAndDrop(vem.$('.o_odoo_studio_community_field_type_container .o_odoo_studio_community_field_tabs'),
            $afterGroupHook, {disableDrop: true});
        assert.containsOnce(vem, '.o_odoo_studio_community_nearest_hook', "There should be 1 highlighted hook");
        await testUtils.dom.dragAndDrop(vem.$('.o_odoo_studio_community_field_type_container .o_odoo_studio_community_field_columns'),
            $afterGroupHook, {disableDrop: true});
        assert.containsOnce(vem, '.o_odoo_studio_community_nearest_hook', "There should be 1 highlighted hook");
    });
    ...
    QUnit.test('One2Many list editor column_invisible in attrs ', async function (assert) {
        ...
        await testUtils.dom.click($(target).find('.o_odoo_studio_community_editX2Many[data-type="list"]'));
        await legacyExtraNextTick();

        await testUtils.dom.click($(target).find('.o_odoo_studio_community_sidebar').find('.o_odoo_studio_community_view'));
        await testUtils.dom.click($(target).find('.o_odoo_studio_community_sidebar').find('input#show_invisible'));

        // select the first column
        await testUtils.dom.click($(target).find('thead th[data-studio-xpath]'));
        // enable readonly
        await testUtils.dom.click($(target).find('.o_odoo_studio_community_sidebar').find('input#readonly'));
    });
    ...
    QUnit.test("One2Many form datapoint doesn't contain the parent datapoint", async function (assert) {
        ...
        */
        ...
        await testUtils.dom.click($(target).find('.o_odoo_studio_community_form_view_editor .o_field_one2many'));
        await testUtils.dom.click(
            $(target).find('.o_odoo_studio_community_form_view_editor .o_field_one2many .o_odoo_studio_community_editX2Many[data-type="form"]')
        );
        await legacyExtraNextTick();
    });
    ...
    QUnit.test("navigate in nested x2many which has a context", async function (assert) {
        ...
        await click(target.querySelector('.o_odoo_studio_community_form_view_editor .o_field_one2many'));
        await click(
            target.querySelector('.o_odoo_studio_community_form_view_editor .o_field_one2many .o_odoo_studio_community_editX2Many[data-type="form"]')
        );
        await legacyExtraNextTick();
    ...
    QUnit.test('folds/unfolds the existing fields into sidebar', async function (assert) {
        ...
        assert.containsN(vem, '.o_odoo_studio_community_field_type_container', 3,
            "there should be three sections in Add (new & existing fields & Components");
        assert.hasClass(vem.el.querySelector('.o_odoo_studio_community_existing_fields_icon'), 'fa-caret-right',
            "should have a existing fields folded");
        assert.isNotVisible(vem.el.querySelector('.o_odoo_studio_community_existing_fields_section'),
            "the existing fields section should not be visible");

        // Unfold the existing fields section
        await testUtils.dom.click(vem.el.querySelector('.o_odoo_studio_community_existing_fields_icon'));

        assert.containsN(vem, '.o_odoo_studio_community_field_type_container', 3,
            "there should be three sections in Add (new & existing fields & Components");
        assert.hasClass(vem.el.querySelector('.o_odoo_studio_community_existing_fields_icon'), 'fa-caret-down',
            "should have a existing fields unfolded");
        assert.isVisible(vem.el.querySelector('.o_odoo_studio_community_existing_fields_section'),
            "the existing fields section should be visible");

        // drag and drop the new char field
        await testUtils.dom.dragAndDrop(vem.el.querySelector('.o_odoo_studio_community_existing_fields .o_odoo_studio_community_field_char'),
            vem.el.querySelector('.o_inner_group .o_odoo_studio_community_hook'));
        assert.isVisible(vem.el.querySelector('.o_odoo_studio_community_existing_fields_section'),
    ...
    QUnit.test('open xml editor of component view', async function (assert) {
        ...
        await testUtils.dom.click(vem.$('.o_odoo_studio_community_xml_editor'));
        await xmlEditorDef;
    ...
    QUnit.test('existing field section should be unfolded by default in kanban', async function (assert) {
        ...
        await testUtils.dom.click(vem.el.querySelector('.o_odoo_studio_community_new'));
        assert.hasClass(vem.el.querySelector('.o_odoo_studio_community_existing_fields_icon'), 'fa-caret-down',
            "should have a existing fields unfolded");
        assert.isVisible(vem.el.querySelector('.o_odoo_studio_community_existing_fields_section'),
            "the existing fields section should be visible");
    ...
    QUnit.test('open xml editor of graph component view and close it', async function (assert) {
        ...
        assert.containsOnce(vem, '.o_ace_view_editor', "the XML editor should be opened");
        assert.containsNone(vem, '.o_odoo_studio_community_sidebar');
