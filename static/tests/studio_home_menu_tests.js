...
import { StudioHomeMenu } from "@odoo_studio_community/client_action/studio_home_menu/studio_home_menu";
import { MODES } from "@odoo_studio_community/studio_service";
...
QUnit.module("Studio", (hooks) => {
    ...
    QUnit.test("simple rendering", async (assert) => {
        ...
        assert.containsOnce(firstApp, ".o_odoo_studio_community_edit_icon i");
        ...
        assert.containsOnce(secondApp, ".o_odoo_studio_community_edit_icon i");

        // New app button
        assert.containsOnce(
            target,
            "div.o_apps > a.o_app.o_odoo_studio_community_new_app",
            'should contain a "New App icon"'
        );
        const newApp = target.querySelector("a.o_app.o_odoo_studio_community_new_app");
        assert.strictEqual(
            newApp.querySelector("img.o_app_icon").dataset.src,
            "/odoo_studio_community/static/src/img/default_icon_app.png",
            "Image source URL should end with '/odoo_studio_community/static/src/img/default_icon_app.png'"
        );
    ...
    QUnit.test("Click on new App", async (assert) => {
        ...
        await click(target, "a.o_app.o_odoo_studio_community_new_app");
    });
    ...
    QUnit.test("Click on edit icon button", async (assert) => {
        ...
        const firstEditIconButton = target.querySelector(".o_odoo_studio_community_edit_icon i");
        ...
        assert.containsOnce(
            dialog,
            ".modal-content.o_odoo_studio_community_edit_menu_icon_modal .o_odoo_studio_community_icon_creator"
        );
    ...
    QUnit.test("edit an icon", async (assert) => {
        ...
        const mockRPC = (route, args) => {
            if (route === "/odoo_studio_community/edit_menu_icon") {
        ...
        await click(target.querySelector(".o_odoo_studio_community_edit_icon i"));
        const dialog = document.querySelector("div.modal");
        await click(dialog.querySelector(".o_odoo_studio_community_upload a"));

        assert.doesNotHaveClass(
            dialog.querySelector(".o_odoo_studio_community_icon .o_app_icon i"),
            "fa-balance-scale"
        );

        // Change the icon's pictogram
        await click(dialog.querySelectorAll(".o_odoo_studio_community_selector")[2]);
        await click(dialog, ".o_odoo_studio_community_selector .fa.fa-balance-scale");
