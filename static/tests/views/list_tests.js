...
import { registerStudioDependencies } from "@odoo_studio_community/../tests/helpers";
import { patchListRendererStudio } from "@odoo_studio_community/views/list/list_renderer";
...
QUnit.module("Studio", (hooks) => {
    hooks.beforeEach(() => {
        ...
        patch(ListRenderer.prototype, "odoo_studio_community.ListRenderer", patchListRendererStudio);
    });
    ...
    QUnit.test("add custom field button with other optional columns", async function (assert) {
        ...
        assert.containsOnce(
            target,
            ".o_studio .o_odoo_studio_community_editor .o_odoo_studio_community_list_view_editor"
        );
    });
    ...
    QUnit.test("should render the no content helper of studio actions", async function (assert) {
        const webClient = await createEnterpriseWebClient({ serverData });
        await doAction(webClient, 3);
        await click(target.querySelector(".o_odoo_studio_community_navbar_item a"));
        await legacyExtraNextTick();
        const automationsLink = [...target.querySelectorAll(".o_odoo_studio_community_menu_item a")].find(
