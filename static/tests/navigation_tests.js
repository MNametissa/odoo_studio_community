...
import { leaveStudio, openStudio, registerStudioDependencies } from "@odoo_studio_community/../tests/helpers";
import { registry } from "@web/core/registry";
import { session } from "@web/session";
import { patch, unpatch } from "@web/core/utils/patch";
import { StudioView } from "@odoo_studio_community/client_action/studio_view";
import { StudioClientAction } from "@odoo_studio_community/client_action/studio_client_action";
import { ListEditorRenderer } from "@odoo_studio_community/client_action/view_editors/list/list_editor_renderer";
...
QUnit.module("Studio", (hooks) => {
    ...
    QUnit.test("Studio not available for non system users", async function (assert) {
        ...
        assert.containsNone(target, ".o_main_navbar .o_odoo_studio_community_navbar_item a");
    });
    ...
    QUnit.test("open Studio with act_window", async function (assert) {
        ...
        assert.containsOnce(target, ".o_main_navbar .o_odoo_studio_community_navbar_item a");

        await openStudio(target);

        assert.verifySteps(
            [
                "/odoo_studio_community/activity_allowed",
                "/odoo_studio_community/get_studio_view_arch",
                "/web/dataset/call_kw/partner/get_views",
                "/web/dataset/call_kw/partner/web_search_read",
            ],
        ...
        assert.containsOnce(
            target,
            ".o_odoo_studio_community_client_action .o_odoo_studio_community_kanban_view_editor",
        ...
        assert.containsOnce(target, ".o_studio_navbar .o_odoo_studio_community_leave a");
        ...
        assert.containsNone(target, ".o_odoo_studio_community_client_action", "Studio should be closed");
        assert.containsOnce(
            target,
    ...
    QUnit.test("open Studio with act_window and viewType", async function (assert) {
        ...
        assert.containsOnce(
            target,
            ".o_odoo_studio_community_client_action .o_odoo_studio_community_form_view_editor",
    ...
    QUnit.test("reload the studio view", async function (assert) {
        ...
        patch(StudioView.prototype, "odoo_studio_community.Test.StudioView", {
            setup() {
                this._super();
                owl.onMounted(() => {
                    prom.resolve();
        ...
        await prom;
        unpatch(StudioView.prototype, "odoo_studio_community.Test.StudioView");
    ...
    QUnit.test("switch view and close Studio", async function (assert) {
        ...
        await openStudio(target);
        assert.containsOnce(target, ".o_odoo_studio_community_client_action .o_odoo_studio_community_kanban_view_editor");

        // click on tab "Views"
        await click(target.querySelector(".o_odoo_studio_community_menu .o_odoo_studio_community_menu_item a"));
        assert.containsOnce(target, ".o_odoo_studio_community_action_editor");

        // open list view
        await click(
            target.querySelector(
                ".o_odoo_studio_community_views .o_odoo_studio_community_view_type[data-type=list] .o_odoo_studio_community_thumbnail"
            )
        );
        await legacyExtraNextTick();

        assert.containsOnce(target, ".o_odoo_studio_community_client_action .o_odoo_studio_community_list_view_editor");
    ...
    QUnit.test("navigation in Studio with act_window", async function (assert) {
        ...
        assert.containsOnce(
            target,
            ".o_odoo_studio_community_client_action .o_odoo_studio_community_list_view_editor",
    ...
    QUnit.test("open same record when leaving form", async function (assert) {
        ...
        assert.containsOnce(target, ".o_odoo_studio_community_client_action .o_odoo_studio_community_form_view_editor");
    ...
    QUnit.test("open Studio with non editable view", async function (assert) {
        ...
        assert.containsOnce(
            target,
            ".o_odoo_studio_community_action_editor",
    ...
    QUnit.test("kanban in studio should always ignore sample data", async function (assert) {
        ...
        assert.containsOnce(
            target,
            ".o_odoo_studio_community_kanban_view_editor .o_kanban_group .o_kanban_record:not(.o_kanban_ghost):not(.o_kanban_demo)",
            "the kanban view should not contain any sample data"
        );

        assert.containsNone(target, "o_odoo_studio_community_kanban_view_editor .o_view_nocontent");
    });
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            await click(
                target.querySelector(
                    '.o_odoo_studio_community_form_view_editor .o_field_one2many .o_odoo_studio_community_editX2Many[data-type="form"]'
                )
            );
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            webClient.env.bus.on("ACTION_MANAGER:UI-UPDATED", null, () => {
                assert.containsOnce(target, ".o_kanban_view");
                assert.hasClass(target.querySelector(".o_odoo_studio_community_navbar_item"), "o_disabled");
                def.resolve();
            });
    ...
    QUnit.test("command palette inside studio", async (assert) => {
        ...
        assert.containsOnce(target, ".o_studio .o_odoo_studio_community_kanban_view_editor");
    });
});
