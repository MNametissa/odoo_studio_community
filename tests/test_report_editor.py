...
class TestReportEditor(TransactionCase):
    ...
    def test_copy_inherit_report(self):
        report = self.env['ir.actions.report'].create({
            ...
            'report_name': 'odoo_studio_community.test_inherit_report_user',
            'model': 'res.users',
        })
        self.env['ir.ui.view'].create({
            'type': 'qweb',
            'name': 'odoo_studio_community.test_inherit_report_hi',
            'key': 'odoo_studio_community.test_inherit_report_hi',
            'arch': '''
                <t t-name="odoo_studio_community.test_inherit_report_hi">
                    hi
                </t>
            ''',
        })
        parent_view = self.env['ir.ui.view'].create({
            'type': 'qweb',
            'name': 'odoo_studio_community.test_inherit_report_user_parent',
            'key': 'odoo_studio_community.test_inherit_report_user_parent',
            'arch': '''
                <t t-name="odoo_studio_community.test_inherit_report_user_parent_view_parent">
                    <t t-call="odoo_studio_community.test_inherit_report_hi"/>!
                </t>
            ''',
        })
        self.env['ir.ui.view'].create({
            'type': 'qweb',
            'name': 'odoo_studio_community.test_inherit_report_user',
            'key': 'odoo_studio_community.test_inherit_report_user',
            'arch': '''
        ...
        copy_report = self.env['ir.actions.report'].search([
            ('report_name', '=', 'odoo_studio_community.test_inherit_report_user_copy_1'),
        ])
    ...
    def test_duplicate(self):
        ...
        report = self.env['ir.actions.report'].create({
            'name': 'test duplicate',
            'report_name': 'odoo_studio_community.test_duplicate_foo',
            'model': 'res.users',})

        self.env['ir.ui.view'].create({
            'type': 'qweb',
            'name': 'test_duplicate_foo',
            'key': 'odoo_studio_community.test_duplicate_foo',
            'arch': "<t t-name='odoo_studio_community.test_duplicate_foo'>foo</t>",})

        duplicate_domain = [('report_name', '=like', 'odoo_studio_community.test_duplicate_foo_copy_%')]
    ...
    def test_duplicate_keep_translations(self):
        def create_view(name, **kwargs):
            ...
            name = 'odoo_studio_community.test_keep_translations_{}'.format(name)
        ...
        report = self.env['ir.actions.report'].create({
            'name': 'test inherit report user',
            'report_name': 'odoo_studio_community.test_keep_translations_ab',
            'model': 'res.users',
        ...
        report.copy_report_and_template()
        copy_view = self.env['ir.ui.view'].search([
            ('key', '=', 'odoo_studio_community.test_keep_translations_ab_copy_1'),
        ])
...
class TestReportTranslation(HttpCase):
    ...
    def test_report_edit_keep_translation(self):
        ...
        self.env['ir.actions.report'].create({
            ...
            'report_name': 'odoo_studio_community.test_report',
            'model': 'res.users',
        }).with_context(load_all_views=True)
        view = self.env['ir.ui.view'].create({
            'type': 'qweb',
            'name': 'test_report_view',
            'key': 'odoo_studio_community.test_report_view',
            'arch': '<t t-name="odoo_studio_community.test_report_view"><div>hello test</div></t>',
        })
        ...
        res = self.url_open(
            '/odoo_studio_community/get_report_views',
            data=json.dumps({"params": {'report_name': 'odoo_studio_community.test_report_view', 'record_id': user.id}}),
        ...
        res = self.url_open(
            '/odoo_studio_community/edit_report_view_arch',
            data=json.dumps({"params": {
                'report_name': 'odoo_studio_community.test_report_view',
                'record_id': user.id,
                'view_id': view.id,
                'view_arch': '<t t-name="odoo_studio_community.test_report_view"><div>hello test</div><div>hi test</div></t>'}}),
            headers={"Content-Type": "application/json"},
        )
        view_arch = etree.fromstring(json.loads(res.content.decode("utf-8"))['result']['views'][str(view.id)]['arch'])
        div_node = view_arch.xpath('//div')
